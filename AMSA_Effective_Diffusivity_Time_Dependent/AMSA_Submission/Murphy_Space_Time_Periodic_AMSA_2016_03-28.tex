...
The signed measures $\Real\mu_{\psi\varphi}$ and $\Imag\mu_{\psi\varphi}$ shown in
...
advection-diffusion were proposed in~\cite{Pavliotis:PHD_Thesis} 
...
we generalize the approach proposed in~\cite{Pavliotis:PHD_Thesis},
...
$\|\bnabla\cdot\|_{\Vc}$ of the space of all twice continuously
...
L^2(\Vc)$ defined by $\partial f/\partial x_j=\lim_{n\to\infty}\partial f_n/\partial x_j$ where the $f_n\in
...
fundamental solution for the Laplacian that vanishes on the boundary
$\partial\Vc$ of the spatial region $\Vc$ and a harmonic function with,
...
$f,h\in\Hs^{1,2}_{\Vc}$ and let $\{ f_n\}$ be a sequence of
$\Vc$--periodic functions that is Cauchy the norm
$\langle|\bnabla\cdot|^2\rangle_{\Vc}$ and converge to $f$ in
$L^2(\Vc)$. Then, we have that (see Theorem 1 in Section 4.2
of~\cite{McOwen:2003:PDE})      
%
\begin{align}\label{eq:Weak_Delta_1}
  \langle(-\Delta)(-\Delta)^{-1}f,h\rangle_{\Vc}
  &\equiv\lim_{n\to\infty}\left\langle\int_{\Vc} G(\vecx-\vecy)(-\Delta_y)f_n(\vecy)\d\vecy,h\right\rangle_{\Vc}
  \\\notag
...
and since the the boundary terms
$\int_{\partial\Vc}[f_n(\vecy)\,\partial G(\vecx-\vecy)/\partial\mathbf{n}_y
-G(\vecx-\vecy)\,\partial f_n(\vecy)/\partial\mathbf{n}_y]\,\d S_y$~\cite{McOwen:2003:PDE} vanish by   
...
$\|f\,g\|_1\leq\|f\|_{p_1}\,\|g\|_{q_1}$, with conjugate exponends
...
the function $x^{\,p_1}$ is convex for $x>0$. Therefore, by Jenson's
...
with conjugate exponends satisfying $(1/p_2)+(1/q_2)=1$ and $1\leq
p_2,\,q_2\leq\infty$. 
...
\end{align}
%
with conjugate exponends satisfying $(1/p_3)+(1/q_3)=1$ and $1\leq
p_3,\,q_3\leq\infty$.
...
such as $\max(r,\hat{r})$ etc. However, recognising that the values of
...
%
We first establish that the operator $M=-\imath A$ with domain $\Fs$ is
self-adjoint. We have already established
...
recalling that $\Vc=[0,\ell]^d$, the calculation in
equation~\eqref{eq:L2_to_H1} and the Poincar{\'e} 
inequality~\cite{McOwen:2003:PDE}, $\|f\|_{\Vc}\leq2\ell\|\bnabla f\|_{\Vc}$, show that the
operator $(-\Delta)^{-1}$ is bounded on $\Hs^{1,2}_{\Vc}$ with operator norm
bounded by the quantity $2\ell\sqrt{C}$. It is also
...
We now summarize our findings. Define the operator $M=-\imath A$, where
$A=(-\Delta)^{-1}(\partial_t-\vecu\bcdot\bnabla)$. We have established that the
operator $-\imath(-\Delta)^{-1}\partial_t$ with domain $\Fs$ self-adjoint and the
operator $-\imath(-\Delta)^{-1}[\vecu\bcdot\bnabla]$ with domain $\Hs$ is
self-adjoint when $u_j\in\tilde{\As}^{\,0}_{\Tc}\otimes (\Hs_{\Vc}\cap L^r(\Vc))$
for $2<r\leq\infty$. Consequently, the difference $M$ of these two operators
with domain $D(M)=\Fs\cap\Hs=\Fs$~\cite{Stone:64} is self-adjoint when 
...
For $u_j\in\tilde{\As}^{\,0}_{\Tc}\otimes\Hs_{\Vc}$ the function
$g_j=(-\Delta)^{-1}u_j$ satisfies $\langle g_j(\cdot,\vecx)\rangle_{\Tc}=0$ for all
$\vecx\in\Vc$, hence by the Fubini
theorem~\cite{Folland:99:RealAnalysis} we have $\langle g_j\rangle=0$. Moreover,
since the operator $(-\Delta)^{-1}$ maps $\Hs_{\Vc}$ to
$\Hs^{1,2}_{\Vc}\cup\mathbb{C}$ we have $g_j=(-\Delta)^{-1}u_j\in\Fs$. The
conditions of the spectral theorem are thus satisfied. Consequently,
the integral representations in equation~\eqref{eq:Spectral_Theorem}
hold for the functions $F(\lambda)$ and $G(\lambda)$ defined above, involving the
complex measure $\mu_{jk}$. The discussion leading to
...
equations~\eqref{eq:Eff_Diffusivity_Sobolev}--\eqref{eq:Integral_Rep_kappa*} involving the \emph{curl-free} vector field $\bnabla\chi_j$ shown in
equation~\eqref{eq:Periodic_Cell_Prob}, with suitable notational
changes, and a maximal (skew-symmetric) 
normal operator acting on a suitable Hilbert space.
...
$\tilde{\As}^{\,0}_{\Tc}$ given in equation~\eqref{eq:Abs_Cont_Mean_Zero}.
Now define
...
$\bGamma_\bullet=\bnabla\btimes(-\bDelta)^{-1}\bnabla \btimes\,$, and $\bGamma_0=\langle\cdot\rangle$, 
...
simplicity. We denote by $\langle\cdot,\cdot\rangle_\times$ the sesquilinear inner-product
associated with the Hilbert space $\Hc_\times$, where
$\langle\vecpsi,\vecvarphi\rangle_\times=\left\langle\vecpsi\bcdot\vecvarphi\right\rangle_{\Vc}$,
which induces a norm $\|\cdot\|_{\times}$ given by
$\|\vecpsi\|_{\times}=\langle\vecpsi,\vecpsi\rangle_\times^{1/2}$.
If $f\in\Hs^{1,2}_{\Vc}$, equation~\eqref{eq:Weak_Delta_1} shows that the
vector field $\bnabla f$  is curl free, $\bnabla f\in\Hc_\times$,  in the
following weak sense. Recall that the Sobolev space $\Hs^{1,2}_{\Vc}$
...
in $\Hs_{\Vc}$~\cite{Bhattacharya:AAP:1999:951}. For arbitrary
$\bpsi\in\Hc_\times$ and for a sequence $\{ f_n\}$ of $\Vc$--periodic functions
$f_n\in C^2(\Vc)$ that is Cauchy in the norm $\langle|\bnabla\cdot|^2\rangle_{\Vc}$ that
converge to  $f$ in $L^2(\Vc)$ we have  
%
\begin{align}\label{eq:Gamma_Proj_H12}
  \langle\bGamma\bnabla f,\bpsi\rangle_\times
  =\lim_{n\to\infty}\langle\bnabla(-\Delta)^{-1}(-\Delta)f_n,\bpsi\rangle_\times
  =\lim_{n\to\infty}\langle\bnabla f_n,\bpsi\rangle_\times
  =\langle\bnabla f,\bpsi\rangle_\times.
\end{align}
%
% %
...
norm and trivially symmetric. This
establishes a link between the Hilbert spaces $\Hs^{1,2}_{\Vc}$ and
$\Hc_\times$. The following lemma shows that these Hilbert spaces
are in one-to-one isometric correspondence, which establishes that 
$\Hc_\times\equiv\{\bnabla f\in\Hc_{\Vc} \,|\, f\in\Hs^{1,2}_{\Vc}\}$. This, in turn,
establishes that the linear symmetric operator $\bGamma$ with domain
$\Hc_\times$ is self-adjoint.   
...
%  
  ...
  $\Hs^{1,2}_{\Vc}\sim\Hc_\times$. More specifically, temporarily denote the
  inner-product induced norm of the Hilbert space $\Hs^{1,2}_{\Vc}$
  by $\|f\|_{1,2}=\langle\bnabla f\bcdot\bnabla f\rangle_{\Vc}^{1/2}$. Then for every
...
weekly so that $\bnabla f\in\Hc_\times$. Moreover,
...
\begin{align}\label{eq:Function_Space_Vector} 
  ...
  %\langle \psi,g\rangle_{1,2}=\left\langle\overline{\bnabla \psi}\bcdot\bnabla g\right\rangle,
\end{align}
%
...
For notational simplicity, we will drop the subscripts $\xi$ shown
...
%
We use the formula $\vecu =\bnabla \bcdot\Hm$ shown in
...
$\;\vecg_j=-\bGamma\Hm\vece _j$, 
shown in equations,~\eqref{eq:Dt_T}, ~\eqref{eq:StoA},
...
characterization of the spatial variable $\vecx$, the one-to-one
isometry between $\Hs^{1,2}_{\Vc}$ and $\Hc_\times$ established in
\lemref{lem:Isometry_Fs_Fc} 
induces a one-to-one isometry between $\Fs$ and $\Fc$. We now
...
$D(A)=D(M)$. We already established in \appref{app:Scalar_Fields} that
$\Fs\subset D(A)$, and similarly for the operator $\Ab$.
