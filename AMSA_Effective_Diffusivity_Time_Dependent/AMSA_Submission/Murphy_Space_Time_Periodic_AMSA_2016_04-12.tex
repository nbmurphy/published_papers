...
velocity field component arising the right hand side of the cell
...
However the function $u_j$ appearing on the right hand side
...
hand side of equation~\eqref{eq:Periodic_Cell_Prob} is
\emph{mean-zero}. Consequently, the Stieltjes integral representations
for $\Sm^*$ and $\Am^*$ discussed in the next section hold for all of
...
\begin{align}\label{eq:Spec_Weights}
  m_{jk}(l)=\langle u_j,\varphi_l\rangle\,\overline{\langle u_k,\varphi_l\rangle},
  \qquad
  \langle u_j,\varphi_l\rangle=\langle u_j\;\overline{\varphi_l}\rangle.
\end{align}
%
...
$\Dm^*_{jk}\equiv\tilde{\Dm}^*_{jk}$. It is worth noting that writing the
spectral weights 
$m_{jk}(l)$ in equation~\eqref{eq:Spec_Weights} in Dirac notation $\langle
u_j,\varphi_l\rangle\,\overline{\langle
  u_k,\varphi_l\rangle}=\langle u_j|\,(|\varphi_l\rangle\langle\varphi_l|)\,|u_k\rangle=\langle u_j|Q_l|u_k\rangle$, with
$Q_l=|\varphi_l\rangle\langle\varphi_l|$ heuristically identifies the projection operator
$Q(\lambda)$ as  
%
\begin{align}
  Q(\lambda)=\sum_{l:\;\lambda_l\leq\lambda}\theta(\lambda-\lambda_l)\;Q_l,
  \quad
  \d\mu_{jk}(\lambda)=\sum_{l:\;\lambda_l\leq\lambda}\delta_{\lambda_l}(\d\lambda)\langle u_j|Q_l|u_k\rangle,
\end{align}
%
where $\theta(\lambda)$ is the Heaviside function and $\delta_{\lambda_l}(\d\lambda)$ is the Dirac
$\delta$-measure concentrated at $\lambda_l$. This formula is precisely true
for the matrix setting~\cite{Murphy:2015:CMS:13:4:825}. 
...
inner-product $\langle\cdot,\cdot\rangle$ is given by $\langle f,h\rangle=\langle f\;\overline{h}\rangle$. Since the 
eigenfunction $\varphi_l$ in~\eqref{eq:Trig_phi_uj} is mean-zero in both
space and time, we have 
$\ell\in\mathbb{Z}\backslash\{0\}$ and $\veck\in\mathbb{Z}^d\backslash\{0\}$. Assuming that the
mean-zero components $u_j$, $j=1,\ldots,d$, of $\vecu$ are expressible as
\emph{finite} Fourier series, the indices $(\ell^\prime,\,\veck^\prime)$
in~\eqref{eq:Trig_phi_uj} run over  the index sets $\ell^\prime\in I_N^1$
and $\veck^\prime\in I_N^d$ where 
$I_N^n$ is defined as $I_N^n=\{\vecq\in\mathbb{Z}^n\backslash\{0\}   \;|\, -N\leq q_i\leq
N,  \;N\in\mathbb{N}\}$.
...
\bigg]
\notag\\
&=\imath\lambda_la^{\,l}_{\ell,m,n}, \qquad \ell\neq0, \quad (m,n)\neq(0,0).
\notag
\end{align}
%
...
\begin{align}\label{eq:Inner_Products_2D}
  \langle u_1,\varphi_l\rangle=\frac{1}{2}\left(a^{\,l}_{0,0,1}+a^{\,l}_{0,0,-1}\right)
      +\frac{\theta}{4\imath}\left(a^{\,l}_{1,0,1}-a^{\,l}_{1,0,-1}+a^{\,l}_{-1,0,1}-a^{\,l}_{-1,0,-1}\right), 
               \\
  \langle u_2,\varphi_l\rangle=\frac{1}{2}\left(a^{\,l}_{0,1,0}+a^{\,l}_{0,-1,0}\right)
...
\begin{align}\label{eq:Inner_Products_2D_Steady}
  &\langle u_1,\varphi_l\rangle=\frac{1}{2}\left(a^{\,l}_{0,1}+a^{\,l}_{0,-1}\right),
  \quad
  \langle u_2,\varphi_l\rangle=\frac{1}{2}\left(a^{\,l}_{1,0}+a^{\,l}_{-1,0}\right).
\end{align}
%
...
and the eigen-vectors $\veca_l$ -- consisting of the Fourier
...
Since the index sets are restricted to $\ell\neq0$ and $(m,n)\neq0$, the matrix
$\Cm$ is strictly positive definite and diagonal. Consequently, the
...
$N=20$, yielding matrices of size $2N[(2N+1)^2-1]=67,200$. The
...
$\Delta$ maps constants to the null element 0 and $\Vc$--periodic functions
to $\Vc$--periodic functions. Hence, $\Delta$ maps mean-zero $\Vc$--periodic
functions to mean-zero $\Vc$--periodic
functions. In the cell problem in
equation~\eqref{eq:Periodic_Cell_Prob} the   
...
$(-\Delta)^{-1}$ is mean-zero $\Vc$--periodic functions. The function
$\partial_t\chi_j$ in~\eqref{eq:Periodic_Cell_Prob} is therefore mean-zero
in space and also mean-zero
in time by the temporal periodicity of $\chi_j$. Since, the fluid
velocity field $\vecu$ is divergence free, $\bnabla\bcdot\vecu=0$, the
function $\vecu\bcdot\bnabla\chi_j=\bnabla\bcdot(\vecu\chi_j)$
in~\eqref{eq:Periodic_Cell_Prob} is  mean-zero
in space by the divergence 
theorem~\cite{McOwen:2003:PDE} and the spatial periodicity of
$\vecu$ and $\chi_j$. This establishes that all of the terms on the left side of
equation~\eqref{eq:Periodic_Cell_Prob} are mean-zero in space and
time, which requires that the function $u_j$ on the right side
of~\eqref{eq:Periodic_Cell_Prob} is also mean-zero in space and time.
...
% $u_j\in\tilde{\As}^{\,0}_{\Tc}\otimes(\Hs_{\Vc}\cap L^r(\Vc))$ for $2<r\leq\infty$. 
...
\end{align}
%
where $\ell\in\mathbb{Z}\backslash\{0\}$ and $\veck\in\mathbb{Z}^d\backslash\{0\}$, as
...
\end{align}
%
where $\ell\in\mathbb{Z}\backslash\{0\}$, and $\veck\in\mathbb{Z}^d\backslash\{0\}$.
