...
\bigg]
\notag\\
&=\lambda_la^{\,l}_{\ell,m,n}, \qquad \ell\neq0, \quad (m,n)\neq(0,0).
\notag
\end{align}
%
...
Before proving \thmref{thm:Integral_Reps}, we discuss the key
properties of the linear operator $(-\Delta)^{-1}$ and justify the
calculation in equation~\eqref{eq:uj_chik}. The Laplacian
...
in time by the temporal periodicity of $\chi_j$ and the fundamental
theorem of calculus. Since, the fluid
...
\end{align}
%
The Green's function $G$ can be constructed~\cite{Melnikov:2012:Green} in
