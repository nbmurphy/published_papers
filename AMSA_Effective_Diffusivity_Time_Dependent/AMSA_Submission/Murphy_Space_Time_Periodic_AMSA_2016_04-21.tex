...
In \appref{sec:Trig:Fourier_Methods} we demonstrate that a Fourier 
...
%$\phi_{\ell,\,\veck}(t,\vecx)=\exp[\imath(\ell t+\veck\bcdot\vecx)]$,
the
...
cell $\Tc\times\Vc$. In \appref{app:Scalar_Fields} we show it is necessary
...
\thmref{thm:Integral_Reps} in \appref{app:Scalar_Fields} for
...
\appref{app:Hilbert_Resolvent_Integral_Reps} below. Consider the
...
while in~\appref{app:Curl_Free_Fields} we extend the approach
proposed
...
\end{align}
%
% For $f$ is also spatially mean-zero since $L^2(\Vc)\subset L^1(\Vc)$
% implies that $|\langle f\rangle_{\Vc}|=|\langle f-f_n\rangle_{\Vc}|\leq\|f-f_n\|_1\to0$ as $n\to\infty$.
...
theorem discussed in~\appref{app:Spectral_Theory} 
...
proof of \thmref{thm:Integral_Reps} below, we show it is required that
...
\end{align}
%
The following corollary of \thmref{thm:Integral_Reps} establishes the  
Stieltjes integral representations equation
...
established in \lemref{lem:Isometry_Fs_Fc} induces the one-to-one
...
\begin{align}\label{eq:T_Delta_Fs_Fc_SA}
  ...
  %=\langle(-\bDelta)^{-1}\Tb\bpsi\bcdot\bphi\rangle
...
\end{align}
%
This, \lemref{lem:A_bounded}, and \lemref{lem:Isometry_Fs_Fc}, in
turn, show that $\bGamma\Hm\bGamma$ is bounded on $\Hc$. 
% when the components $u_j$, $j=1,\ldots,d$, of $\vecu$ satisfy
% $u_j\in\tilde{\As}_{\Tc}\otimes (\Hs^{0,2}_{\Vc}\cap L^r(\Vc))$ for $2<r\leq\infty$. 
...
\lemref{lem:Isometry_Fs_Fc}, imply that $\vecg_j\in\Fc$.   
...
As discussed in the proof of \corref{cor:Integral_Reps}, the
one-to-one isometry $\Hs^{1,2}_{\Vc}\sim\Hc_\times$ established in
\lemref{lem:Isometry_Fs_Fc} induces the one-to-one isometry 
$\Fs\sim\Fc$. By construction, in
Appendices~\numappref{app:Scalar_Fields}
and~\numappref{app:Curl_Free_Fields} we established that $\Fs\subseteq D(A)$
...
\numsecref{app:Scalar_Fields} and \numsecref{app:Curl_Free_Fields}
produce equivalent Stieltjes integral representations for the
effective diffusivity tensor $\Dm^*$. 


In \appref{app:Spectral_Theory} we discussed that the domain $D(M)$ of
...
representations of \thmref{thm:Integral_Reps} are equal to the
measures of \corref{cor:Integral_Reps}, $\d\langle
...
$\vecu$, $j=1,\ldots,d$. Moreover, by \thmref{thm:Integral_Reps} we have
...
$(-\Delta)^{-1}\partial_t\varphi_l\in\Hs_{\Tc}\otimes\Hs^{1,2}_{\Vc}$. By \lemref{lem:A_bounded}
...
statement of \thmref{thm:Integral_Reps} we established the domain of 
...
Section 4.2 of~\cite{McOwen:2003:PDE}). In \thmref{thm:Integral_Reps}
of \appref{app:Scalar_Fields} we established that the operator
