...
%
In this section we employ the theory reviewed in
\secref{sec:Eig_Funct_Exp} to compute the discrete components of the 
Stieltjes integral representations displayed in
equation~\eqref{eq:Integral_Rep_kappa*}, for the fluid velocity field
$\vecu$ in equation~\eqref{eq:tdcell}. Consider the
eigenvalue problem $A\varphi_n=\imath\lambda_n\varphi_n$, $\lambda_n\in\mathbb{R}$, $n\in\mathbb{N}$, 
involving the integro-differential operator
$A=(-\Delta)^{-1}(\partial_t+\vecu\bcdot\bnabla)$ introduced in   
equation~\eqref{eq:Eff_Diffusivity_Sobolev}. Here, for notational
simplicity, we have set $\vecu\to-\vecu$ and
$(\tau,\vecxi)\to(t,\vecx)$. The operator $A$ is a maximal skew-symmetric
(normal) operator with domain $\Fs$ that is an every dense subset of
the Hilbert space $\Hs$, which are defined in
...
require that the elements of $\Hs$ and $\Fs$ are also mean-zero.  



In \secref{sec:Eig_Funct_Exp} we discussed that the span of the
eigenfunctions $\{\varphi_n\}$ form a closed linear manifold
$\Ec=\oplus_{n=1}^\infty\Mc(\varphi_n)$. We also discussed that the Hilbert space $\Hs$
may be written as $\Hs=\Mc\oplus\Nc$, where $\Mc=\Ec\oplus\{0\}$ and $\Nc=\Mc^\perp$
is the orthogonal complement of $\Mc$ in $\Hs$. In
equation~\eqref{eq:Disc_Cont_Measures} we discussed how this
decomposition of the Hilbert space led to a natural 
decomposition of the spectral measure $\mu_{jk}$
in~\eqref{eq:Integral_Rep_kappa*} into its discrete part (associated
with $\Mc$) and its continuous part (associated with $\Nc$). In this
section, we compute the discrete part of the spectral measure, hence
that of the Stieltjes integral representations displayed in
equation~\eqref{eq:Integral_Rep_kappa*}.




Toward this goal, rewrite the
equation $A\varphi_n=\imath\lambda_n\varphi_n$ as         
%
...
underlying the integral representation for the symmetric
...
subsets of their associated Hilbert spaces $\Hc_\times$ and $\Hs$,  
%
\begin{align}\label{eq:Hilbert_Spaces_A}
  \Hc_\times=\{\vecpsi\in \Hc_\Tc\otimes\Hc_\times \,|\, \langle\vecpsi\rangle=0\},\quad
  \Hs=\{f\in \Hs_\Tc\otimes\Hs^1_\Vc \,|\, \langle f\rangle=0\}.
\end{align}
%
...
operator $A$ and the Hilbert space $\Hs$, as the discussion regarding
...
$\Nc=\Mc^\perp$ be the orthogonal complement of $\Mc$ in $\Hs$.  
...
space $\Hs$ in terms of the mutually orthogonal, closed linear manifolds
$\Mc$ and $\Nc$.
%
...
$\Nc(\varphi_m)$ are mutually orthogonal and together determine $\Hs$
according to   
%
\begin{align*}
...
\end{align*}
%
\item $\Ec$ contains a complete orthonormal set $\{\varphi_n\}$; $\Mc=\Hs$
...
$\vecu $. Since $\chi_j,g_j\in\Fs\subset\Hs$, by the formula
$\Hs=\Mc\oplus\Nc$ of Theorem \ref{thm:Hilbert_Eig_Decomp} and equation
...
\cite{Stone:64}. A useful property of the inner-product $\langle\varphi_n,g_k\rangle_1$ and the form of
$g_j=(-\Delta)^{-1}u_j$ is that $\langle\varphi_n,g_j\rangle_1=\langle\varphi_n,u_j\rangle_2$. More
specifically, since $u_j(t,\cdot)\in\Hs(\Vc)\subset L^2(\Vc)$ we have
