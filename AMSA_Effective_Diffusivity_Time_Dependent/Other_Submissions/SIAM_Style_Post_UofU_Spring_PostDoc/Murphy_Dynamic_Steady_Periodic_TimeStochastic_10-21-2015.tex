...
discussed here extend to fluid velocity fields that are expressible as
a finite sum of Fourier modes.




In \secref{sec:Scalar_Fields} we showed that $A$ is a maximal
skew-symmetric (normal) operator with domain $\Fs$ that is an every
dense subset of the Hilbert space $\Hs$ defined in
...
require that the elements of $\Hs$, hence $\Fs$ are also mean-zero,
i.e., $f\in\Hs$ implies that $\langle f\rangle=0$. In \secref{sec:Eig_Funct_Exp} we
discussed that the span of the eigenfunctions $\{\varphi_n\}$ form a closed
linear manifold $\Ec=\oplus_{n=1}^\infty\Mc(\varphi_n)$. We also discussed that the
Hilbert space $\Hs$ may be decomposed as $\Hs=\Mc\oplus\Nc$, where
$\Mc=\Ec\oplus\{0\}$ and $\Nc=\Mc^\perp$ is the orthogonal complement of $\Mc$ in
$\Hs$. This decomposition of the Hilbert space provides a natural
decomposition of the spectral measure $\mu_{jk}$
in~\eqref{eq:Integral_Rep_kappa*} into its discrete part (associated
with $\Mc$) and its continuous part (associated with $\Nc$), as
displayed in equation~\eqref{eq:Disc_Cont_Measures}.  
