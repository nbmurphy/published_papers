...
part $\Sm^*$ plays a role in the homogenized equation
...
$\vecu=\vecu(\vecx)$, the function $\chi_j(\vecx)$ is also
time-independent and satisfies equation~\eqref{eq:Periodic_Cell_Prob}
with $\partial_t\chi_j=0$~\cite{Fannjiang:SIAM_JAM:333,Majda:Kramer:1999:book}. 




Equations~\eqref{eq:phi_bar},~\eqref{eq:Djk},
and~\eqref{eq:Periodic_Cell_Prob} follow from the assumption that the
...
convergence can be rigorously established in the following
sense
%
\begin{align}
...
As mentioned above, in the setting of a mean-zero fluid velocity
field, the effective diffusivity tensor $\Dm^*$ in~\eqref{eq:Djk} is
constant and only its symmetric part $\Sm^*$ plays a role in the
effective transport equation displayed in~\eqref{eq:phi_bar}. More
generally, one could consider a fluid velocity field that has
large-scale structure that varies slowly relative to small-scale,
rapidly varying periodic fluctuations. In this case, the effective
diffusivity tensor $\Dm^*$ can vary slowly in space and
time~\cite{Pavliotis:PHD_Thesis,McLaughlin:Forest:PF:1999:880}, and
both the symmetric $\Sm^*$ and antisymmetric $\Am^*$ parts of $\Dm^*$
play an important role in the effective transport equation. For this
reason, here we will develop Stieltjes integral representations for
both the symmetric and antisymmetric parts of $\Dm^*$. As we will
...
formulation of Stieltjes-Radon integral representations for both the
...
$\xi,\zeta\in\Hs_d$~\cite{Stone:64}. By the sesquilinearity of the
inner-product and the self-adjointness of the projection operator
$Q(\lambda)$, the function $\mu_{\xi\zeta}(\lambda)$ satisfies
$\mu_{\zeta\xi}(\lambda)=\overline{\mu}_{\xi\zeta}(\lambda)$. Moreover, the function $\mu_{\xi\xi}(\lambda)$
is real-valued and positive $\mu_{\xi\xi}(\lambda)=\langle Q(\lambda)\xi,\xi\rangle=\langle
Q(\lambda)\xi,Q(\lambda)\xi\rangle=\|Q(\lambda)\xi\,\|^2\geq0$. 
...
\end{align}
%
where $\Real\mu_{\xi\xi}(\lambda)=\mu_{\xi\xi}(\lambda)$ and $\Imag\mu_{\xi\xi}(\lambda)=0$. With each of
...
\end{align}
%
which we will denote by $\mu_{\xi\xi}$, $\mu_{\xi\zeta}$, $\Real\mu_{\xi\zeta}$, and
$\Imag\mu_{\xi\zeta}$. We stress that $\mu_{\xi\xi}$ is a positive measure, $\mu_{\xi\zeta}$
is a complex measure, while $\Real\mu_{\xi\zeta}$ and $\Imag\mu_{\xi\zeta}$ are signed
...
\begin{align}\label{eq:Spectral_Theorem}
  ...
  \hspace{1em}
  \forall \, \xi\in\mathscr{D}(F), \ \zeta\in\Hs_d,  
  \\
...
\end{align}
%
where $\zeta$ is an arbitrary element in $\Hs_d$~\cite{Stone:64}. In fact,
...
\begin{align}\label{eq:Mass_General}
  ...
  \qquad
  |\mu^0_{\xi\zeta}|\leq\|\xi\|\,\|\zeta\|<\infty.
\end{align}
%
...
\emph{real-valued} so that $\langle F(\Phi)\xi,G(\Phi)\zeta\rangle=\langle G(\Phi)\zeta,F(\Phi)\xi\rangle$. In these
...
\begin{align}\label{eq:Hilbert_Spaces_scalar}
  ...
  \Hs_{\Tc}=\big\{\xi\in L^2(\Tc) \, | \, \xi(t)=\xi(t+\tau)\big\}, \quad
...
$\Tc\times\Vc$. Recalling that $\vecxi\bcdot\veczeta=\vecxi^\dagger\veczeta$,
...
$\langle\xi,\zeta\rangle_1=\left\langle\bnabla \xi\bcdot\bnabla \zeta \right\rangle$ with
...
from the formula $\Dmc^*_{jk}=\varepsilon\delta_{jk}+\langle u_j\chi_k\rangle$ in
...
$\chi_j$ involving the operator $A$ defined
in equation~\eqref{eq:Eff_Diffusivity_Sobolev}
%
...
\end{align}
%
In \secref{sec:Integral_Rep}, we demonstrate that
equations~\eqref{eq:Eff_Diffusivity_Sobolev}
and~\eqref{eq:Resolvent_Rep_Scalar} lead to Stieltjes integral
representations for both $\Sm^*$ and $\Am^*$. 
...
problem~\cite{Avellaneda:PRL-753,Avellaneda:CMP-339} which, instead of
$\Hs^1_{\Vc}$, uses the Hilbert space $\Hc_\times$ of \emph{curl-free}
vector fields, such as the vector field $\bnabla\chi_j$ displayed in
...
$\Ib=\Gamma_\times+\Gamma_\bullet+\Gamma_0$~\cite{Fannjiang:SIAM_JAM:333,MILTON:2002:TC}.
Due to the \emph{curl-free} vector field $\bnabla\chi_j$ displayed in
equation~\eqref{eq:Periodic_Cell_Prob}, we will find particular use of
the Hilbert space $\Hc_\times$, which we define by
%
\begin{align}\label{eq:Hilbert_Curl_Free}
  \Hc_\times=\{\vecxi\in\Hc_{\Vc} \; | \; \Gamma\vecxi=\vecxi\},
  \quad
...
\begin{align}\label{eq:Function_Space_Vector} 
  ...
  \Fc=\{\vecxi\in\tilde{\As}_{\Tc}\otimes\Hc_\times \; | \; \langle \vecxi\rangle=0\},
  %\langle \xi,g\rangle_1=\left\langle\overline{\bnabla \xi}\bcdot\bnabla g\right\rangle,
\end{align}
%
...
by $\langle\vecxi,\veczeta\rangle=\left\langle\vecxi\bcdot\veczeta \right\rangle$
with $\langle\vecxi,\veczeta\rangle=\overline{\langle\veczeta,\vecxi\rangle}$. 


THIS IS WHERE YOU LEFT OF AT 10:49 ON 1-12-2015\\
...
$\vecxi\in\Hc_\times$ implies that $\vecxi=\Gamma\vecxi=\bnabla f$, where
...
$f_2=\Delta^{-1}\bnabla \bcdot\vecxi\,$ then $\Gamma\vecxi=\vecxi$ implies
...
$\langle\Gamma\vecxi\bcdot\veczeta\rangle=\langle\vecxi\bcdot\Gamma\veczeta\rangle$ for 
