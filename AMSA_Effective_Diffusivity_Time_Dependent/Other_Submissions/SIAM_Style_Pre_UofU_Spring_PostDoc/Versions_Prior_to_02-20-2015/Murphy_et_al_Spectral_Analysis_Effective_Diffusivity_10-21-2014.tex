...
effective diffusivity tensor $\Db^*$    
%
...
The components $D^*_{jk}=\Db^*\vec{e}_j\cdot\vec{e}_k$ of the effective
tensor $\Db^*$ are given by $D^*_{jk}=\varepsilon\delta_{jk}+\langle u_j\chi_k\rangle$. For each
...
\begin{align}\label{eq:Symm_Anti-Symm}
  \Db^*=\bkappa^*+\balpha^*,\qquad
  \bkappa^*=\frac{1}{2}\left(\Db^*+[\Db^*]^{\,T}\right), \quad
  \balpha^*=\frac{1}{2}\left(\Db^*-[\Db^*]^{\,T}\right).
\end{align}
%
...
\cite{Stakgold:BVP:2000}.  We stress that, while the effective
diffusivity tensor $\Db^*$ is not symmetric in general, only its
symmetric part appears in the homogenized equation
~\cite{McLaughlin:SIAM_JAM:780} in \eqref{eq:phi_bar}.  
...
field, $D^*_{kk}=\kappa^*_{kk}\geq\varepsilon$. The equality $D^*_{kk}=\kappa^*_{kk}$
...
For the (constant) tensors $\Db^*$ and $\bsig^*$ to be
...
$D^*_{kk}$, $k=1,\ldots,d$, of $\Db^*$ to be well defined and finite,
...
components of the tensors $\Db^*$ and $\bsig^*$ are well defined and
...
%
Let the components $D^*_{jk}$ and $\sigma^*_{jk}$, $j,k=1,\ldots,d$, of the
effective tensors $\Db^*$ and $\bsig^*$ be defined as in equations
...
$D^*_{jk}$ and $\sigma^*_{jk}$ are well defined and finite.
%$|D^*_{jk}|,|\sigma^*_{jk}|<\infty$.
...
\end{align}
%
In particular, the symmetric part $\bkappa^*$ of $\Db^*$ is equal to
that of $\bsig^*$ and the anti-symmetric part $\balpha^*$ of $\Db^*$
...
$\balpha^*$ parts of the effective diffusivity tensor $\Db^*$,
...
that $0<D^*_{kk}<\infty$ for all $0<\varepsilon<\infty$. Consequently, in view of
...
\begin{align}\label{eq:Reduction_final}
  \sigma^*_{jk}=\kappa^*_{jk}-\alpha^*_{jk}=\kappa^*_{kj}+\alpha^*_{kj}=D_{kj}^*\,,       
\end{align}
%
...
symmetric $\bkappa^*$ and anti-symmetric $\balpha^*$ parts of $\Db^*$.      
...
effective diffusivity tensor $\Db^*$, for steady and dynamic
...
integral representation for $\Db^*$, involving a spectral measure
...
representations for the symmetric $\bkappa^*$ and anti-symmetric
$\balpha^*$ parts of the effective diffusivity tensor $\Db^*$, for
...
parameter problem for $\Db^*$ has a more practical numerical
...
$\Db^*$ for model flows. For the case of a time-independent,
...
tensor $\Db^*$ are given by the following functionals 
...
from $D^*_{jk}=\varepsilon\delta_{jk}+\langle u_j\chi_k\rangle$ in
...
formulation of the effective parameter problem for $\Db^*$, we have
...
parts of the effective diffusivity tensor $\Db^*$ for a large class
...
\begin{align}\label{eq:Fourier_Eig_fun}
  ...
  % \qquad
%   u_j(t,\vec{x})=\sum_{(\ell^\prime,\vec{k}^\prime)\in
%     I_M^{d+1}}b^{\,j}_{\ell^\prime,\vec{k}^\prime}\;\e^{\imath(\ell^\prime t+\vec{k}^\prime\cdot\vec{x}\,)}.    
\end{align}
%
...
and anti-symmetric $\balpha^*$ parts of the effective diffusivity
tensor $\Db^*$, displayed in equation (35) of our (attached) paper.  
...
effective diffusivity tensor $\Db^*$. We explore the numerical
...
$D^*_{jk}=\Db^*\vec{e}_j\cdot\vec{e}_k$ of the effective diffusivity
tensor $\Db^*$ given by 
%
\begin{align}\label{eq:Eff_Diffusivity_Appendix}
  D^*_{jk}=\varepsilon\delta_{jk}+\langle u_j\chi_k\rangle.
\end{align}
%
...
effective diffusivity tensor $\Db^*$ emerges. In Section
...
leads to an  integral representation for $\Db^*$, we first discuss
...
directly compute the effective diffusivity tensor $\Db^*$ for model
...
=\sum_{n\in I_N} 
  ...
  \right\rangle,
%                   
% =\sum_{n\in I_N}\left\langle\frac{\Qb_n\vec{g}_j\cdot\vec{g}_k}{\varepsilon^2+\lambda_n^2}\right\rangle,
 %=\int_{-\infty}^\infty\frac{\d\mu_{jk}(\lambda)}{\varepsilon^2+\lambda^2}\,,
 \qquad
\alpha^*_{jk}
% =\sum_{n\in I_N} 
% \left\langle\frac{-\imath\lambda_n\,\overline{(\vec{w}_n^{\,\dagger}\vec{g}_j)}(\vec{w}_n^{\,\dagger}\vec{g}_k)} 
%                      {(\varepsilon-\imath\lambda_n)(\varepsilon+\imath\lambda_n)}\right\rangle.
=\sum_{n\in I_N} 
...
%$[F(A)\vec{\xi}\cdot\vec{\zeta}\,]=\sum_{n=1}^N f(\upsilon_n)[\Qb_n\vec{\xi}\cdot\vec{\zeta}\,]$,
%
...
%=\sum_{n:\;\lambda_n\leq\lambda}\text{Re}[\Qb_n\vec{g}_j\cdot\vec{g}_k\,],
=\frac{1}{2}\sum_{n:\;\lambda_n\leq\lambda}[(\Qb_n+\overline{\Qb}_n)\vec{g}_j\cdot\vec{g}_k\,],
% 
% =\frac{1}{2}\sum_{n:\;\lambda_n\leq\lambda}
%  \langle\delta_{\lambda_n}(\d\lambda)[(\Qb_n+\overline{\Qb}_n)\vec{g}_j\cdot\vec{g}_k]\rangle
%  =\sum_{n:\;\lambda_n\leq\lambda}\langle\delta_{\lambda_n}(\d\lambda)\text{Re}[\Qb_n\vec{g}_j\cdot\vec{g}_k]\rangle,
\\
...
%=\sum_{n:\;\lambda_n\leq\lambda}\text{Im}[\Qb_n\vec{g}_j\cdot\vec{g}_k\,],
...
% \langle\delta_{\lambda_n}(\d\lambda)[(\Qb_n-\overline{\Qb}_n)\vec{g}_j\cdot\vec{g}_k]\rangle
% =\sum_{n:\;\lambda_n\leq\lambda}\langle\delta_{\lambda_n}(\d\lambda)\text{Im}[\Qb_n\vec{g}_j\cdot\vec{g}_k]\rangle.
\notag
\end{align}
%
...
employ the formulation of the effective parameter problem for $\Db^*$
...
$\balpha^*$ of $\Db^*$, and the asymptotics thereof as $\varepsilon\to0$.  
...
\begin{align}\label{eq:Strong_eval_prob_matrix}
  %\imath(\Bnabla^T\Hb\Bnabla-\Db_t)\vec{z}_n=\lambda_n\Bnabla^T\Bnabla\vec{z}_n.
...
%&\langle\Bnabla\vec{\chi}_j\cdot\Bnabla\vec{\chi}_k\rangle=
...
\\
%&\langle\Bb\Bnabla\vec{\chi}_j\cdot\Bnabla\vec{\chi}_k\rangle=
...
\begin{align}\label{eq:Res_of_I_Sobelov}
  \vec{\xi}%=\sum_{n=1}^N\langle\vec{z}_n,\vec{\xi}\,\rangle_1\vec{z}_n
