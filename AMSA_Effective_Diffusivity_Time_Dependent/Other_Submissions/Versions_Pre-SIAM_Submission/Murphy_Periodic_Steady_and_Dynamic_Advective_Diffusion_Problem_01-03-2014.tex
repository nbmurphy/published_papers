...
the Laplacian, $\kappa_0>0$ is the molecular diffusivity, and
$\vec{v}=\vec{v}(t,\vec{x})$ is the fluid velocity field, which is
assumed to be incompressible, mean-zero, and square-integrable,
%
\begin{align}\label{eq:incompressible}
  \vec{\nabla}\cdot\vec{v}=0, \quad
  \langle\vec{v}\,\rangle=0, \quad
  \langle|\vec{v}|^2\rangle<\infty,
\end{align}
%
where we denote by $0$ the null element of all linear spaces in
question and $|\vec{v}|^2=\vec{v}^{\,T}\vec{v}$ is dot-product of the
vector $\vec{v}$ with itself. The bounded sets $\Tc\subset\mathbb{R}$ and   
...
\end{align}
%
where the inverse operation $\Delta^{-1}$ is based on convolution with the
Green's function for the Laplacian $\Delta$ \cite{Stakgold:BVP:2000}.
It is clear from equation \eqref{eq:Eff_Diffusivity} that
$\kappa^*_{jk}=\kappa^*_{kj}$. Due to the fact that the vector field
$\vec{\nabla}\chi_j$ is \emph{real-valued}, the skew-symmetry of the operator
$\Sb:\mathbb{R}^d\mapsto\mathbb{R}^d$ implies that
$\langle\Sb\vec{\nabla}\chi_j\cdot\vec{\nabla}\chi_k\rangle=-\langle\Sb\vec{\nabla}\chi_k\cdot\vec{\nabla}\chi_j\rangle$ and, in particular,
%
...
$.
In the case of steady fluid velocity fields $\bsig=\bkappa$.  
...
this mathematical framework and equation \eqref{eq:Cell_Problem}  to
obtain a resolvent representation of the vector field $\vec{\nabla}\chi_k$,
involving an anti-symmetric integro-differential operator $\Ab$
which is related to the operator $\Sb$ defined in equation
\eqref{eq:Eff_Diffusivity}. Toward this goal, 
...
%
\begin{align}
  \Hs_{\Tc\Vc}=\Hs_{\,\Tc}\otimes\Hs_{\,\Vc}, \qquad
  \Hs_{\,\Tc}=\{ 
     \vec{\xi}\in L^2_d(\Tc)\;|\;
     \vec{\xi}(0)=\vec{\xi}(T) 
                        \}, \quad
...
sesquilinear inner-product associated with 
...
is the complex conjugate of $a\in\mathbb{C}$. By the Helmholtz
theorem \cite{Denaro:2003:0271,Bhatia:IEE:1077}, the Hilbert space
$\Hs_{\,\Vc}$ can be decomposed into orthogonal subspaces of curl-free
$\Hs_\times$, divergence-free $\Hs_\bullet$, and constant $\Hs_{\,0}$ vector
fields, with associated orthogonal projectors
$\bGamma_\times=\vec{\nabla}(\Delta^{-1})\vec{\nabla}\cdot$,
...
\eqref{eq:Cell_Problem} 
and \eqref{eq:Eff_Diffusivity}, we have that $\vec{\nabla}\chi_k$ is (weakly)
curl-free, mean-zero, and $\vec{\nabla}\chi_k\in L^2_d(\Tc)\otimes L^2_d(\Vc)$. Hence,
it is natural for our analysis of the effective parameter problem for
$\Kbc^*$ in \eqref{eq:Eff_Diffusivity}, and that of the associated
...
\end{align}
%
Here, we have used the simplified notation $\langle\vec{\xi}\,\rangle=0\iff\langle\xi_j\rangle=0$ for
all $i=1,\ldots,d$. For the case of time-independent velocity fields
$\vec{u}$, we set $\mathscr{H}_{\Tc}=\emptyset$ so that $\vec{\xi}\in\mathscr{H}$
implies $\vec{\xi}\in\mathscr{H}_\times$ with $\langle\vec{\xi}\,\rangle=0$. We have
already assumed that the flow matrix $\Hb(t,\vec{x})$ is periodic on
$\Tc\otimes\Vc$. We will also assume that it is mean-zero and that $\Hb$ and
its (component-wise) time derivative $\partial_t\Hb$ are bounded in the operator
norm $\|\cdot\|$ induced by the inner-product $\langle\cdot,\cdot\rangle$
\cite{Reed-1980,Stone:64,Stakgold:BVP:2000}      
%
\begin{align}\label{eq:Bounded_H}
  \langle\Hb\rangle=0, \quad \|\Hb\|<\infty, \quad \|\partial_t\Hb\|<\infty. 
\end{align}
%



We now use the Hilbert space framework formulated above to demonstrate
that the effective parameter defined in equation \eqref{eq:sigma*} is
equivalent to that in \eqref{eq:Eff_Diffusivity}, thus putting the
effective parameter problems of advective-diffusive transport and
transport in composite media on common mathematical footing. We first
show that the vector field $\vec{J}_k$ in
\eqref{eq:Maxwells_Equations} is bounded in norm $\|\vec{J}_k\|<\infty$ so
that $\vec{J}_k\in\Hs_\bullet$. Indeed, by equation \eqref{eq:incompressible}
we have that $\|\vec{u}\|<\infty$. Consequently, by the reverse triangle
inequality and equation \eqref{eq:Cell_Problem} (written as in
\eqref{eq:Cell_Prob_Appendix}) we also have that 
% $\infty>\|\vec{u}\|\geq|\|\partial_t\chi_k\|-\|\epsilon\Delta\chi_k+\vec{u}\cdot\vec{\nabla}\chi_k\|$. 
%
\begin{align}\label{eq:Bounded_u}
  \infty>\|u_k\|_2\geq|\|\partial_t\chi_k\|_2-\|\varepsilon\Delta\chi_k+\vec{u}\cdot\vec{\nabla}\chi_k\|_2|, \quad \forall \ k=1,\ldots,d,
\end{align}
%
where we have defined $\|\cdot\|_2$ to be the norm induced by the
$L^2(\Tc\otimes\Vc)$-inner-product. Since the region $\Vc$ is
bounded, the operator $\Delta$ is compact \cite{Stakgold:BVP:2000}, hence
bounded in operator norm $\|\Delta\|<\infty$. so that $\|\varepsilon\Delta\chi_k\|_2<\infty$. Moreover,
also due to the boundedness of $\Vc$, as 
$\vec{u},\vec{\nabla}\chi_k\in L^2_d(\Tc)\otimes L^2_d(\Vc)$ we also have
\cite{Khoshnevisan:2007:Prob} that
$\vec{u},\vec{\nabla}\chi_k\in L^4_d(\Tc)\otimes L^4_d(\Vc)$. The Cauchy-Schwartz
inequality then implies that
$\|\vec{u}\cdot\vec{\nabla}\chi_k\|\leq\||\vec{u}\,|^2\|\,\||\vec{\nabla}\chi_k|^2\|<\infty$. Therefore,
by equation \eqref{eq:Bounded_u} we also have that $\|\partial_t\chi_k\|<\infty$.   

and by equation \eqref{eq:Bounded_H} we have
that $\bkappa$ is bounded in operator norm $\|\bkappa\|<\infty$, and , we have that the vector field 


use the fact that $\vec{\nabla}\chi_k\in\mathscr{H}$ and the Helmholtz
theorem displayed in \eqref{eq:Helmholtz} to show that 

In this
steady case $\bsig\equiv\bkappa=\varepsilon\Ib+\Hb$ which, by 
...
\begin{align}\label{eq:Reduction}
  \sigma^*_{kk}=\bsig^*\vec{e}_j\cdot\vec{e}_k 
       =\langle\bsig\vec{E}_j\cdot\vec{e}_k\rangle
       =\langle\bsig\vec{E}_j\cdot\vec{E}_k\rangle
       =\langle[(\varepsilon\Ib+\Sb)]\vec{E}_j\cdot\vec{E}_k\rangle
       =\varepsilon\langle\vec{E}_k\cdot\vec{E}_k\rangle+\langle\Sb\vec{E}_j\cdot\vec{E}_k\rangle.
       %=\kappa^*_{kk}.
\end{align}
%
...
%
\begin{align}
  \tilde{\Ds}_{\Tc}=\{\xi\in\hat{\Ds}_{\Tc} \,|\, \xi(0)=\xi(T)\}, \quad
  \Ds_{\Tc}=\{\xi\in\hat{\Ds}_{\Tc} \,|\, \xi(0)=\xi(T)=0\}.
\end{align}
%
These sets satisfy $\Ds_{\Tc}\subset\tilde{\Ds}_{\Tc}\subset\hat{\Ds}_{\Tc}$ and
are each everywhere dense in $L^2(\Tc)$ \cite{Stone:64}. Let the
operators $\hat{\Bb}$, $\tilde{\Bb}$, and $\Bb$ be identified as
$\I\partial_t$ with domains $\hat{\Ds}_{\Tc}$, $\tilde{\Ds}_{\Tc}$, and
$\Ds_{\Tc}$, respectively. Then \cite{Stone:64}, $\Bb$ is a closed,
