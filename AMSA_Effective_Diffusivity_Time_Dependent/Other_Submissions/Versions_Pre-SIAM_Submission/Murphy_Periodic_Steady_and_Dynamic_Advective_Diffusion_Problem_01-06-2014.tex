...
It is clear from equation \eqref{eq:Eff_Diffusivity} that the tensor
$\bkappa^*$ is symmetric, $\kappa^*_{jk}=\kappa^*_{kj}$. Due to the fact that
the vector field $\vec{\nabla}\chi_j$ is \emph{real-valued} and that the operator 
...
component-wise time derivative $\Tb\Hb$ is also bounded  
%
...
It follows that the function space
$\Ds_{\Tc}=\otimes_{i=1}^d\tilde{\As}_{\Tc}$ is everywhere dense in
$\Hs_{\,\Tc}$, and it natural to consider the function space
%$\Hs_{\Ds\Vc}\subset\Hs_{\Tc\Vc}$ and $\Hs_{\Ds\times},\Hs_{\Ds\bullet}\subset\Hs_{\Ds\Vc}$
%defined by     
% % 
% \begin{align}\label{eq:Ht}
%   \Hs_{\Ds\Vc}=\Ds_{\Tc}\otimes\Hs_{\Vc}, \qquad
%   \Hs_{\Ds\times}=\Ds_{\Tc}\otimes\Hs_\times, \quad
%   \Hs_{\Ds\bullet}=\Ds_{\Tc}\otimes\Hs_\bullet\,.  
% \end{align}
% %
$\Hs_{\Ds\Vc}=\Ds_{\Tc}\otimes\Hs_{\Vc}$. We stress that $\Hs_{\Ds\Vc}$ is
\emph{not} a Hilbert space, and is instead a dense subset of the
Hilbert space $\Hs_{\Tc\Vc}$. As an operator acting on
$\Hs_{\Ds\Vc}$, $\bsig=\bkappa-(\Delta^{-1})\Tb$ is bounded. This,
$\vec{\nabla}\chi_k\in\Hs_{\,\Tc}\otimes\Hs_\times$, and equation \eqref{eq:Cell_Problem}
suggests that we consider the curl-free mean-zero vector field
$\vec{\nabla}\chi_k$ to be a member of the function space
$\Hs^{\,0}_{\Ds\times}\subset\Hs_{\Ds\Vc}$   
%
\begin{align}\label{eq:Function_Space}
  \Hs^{\,0}_{\Ds\times}=\{\vec{\xi}\in\Ds_{\Tc}\otimes\Hs_\times \,|\, \langle\vec{\xi}\,\rangle=0\},
  %\Hs^{\,0}_{\Ds\times}=\{\vec{\xi}\in\Hs_{\Ds\times} \,|\, \langle\vec{\xi}\,\rangle=0\},
\end{align}
%
which will be used extensively in Section \ref{sec:Integral_Rep}. For
$\vec{\nabla}\chi_k\in\Hs^{\,0}_{\Ds\times}$ the divergence-free vector field
$\vec{J}_k$ is bounded $\|\vec{J}_k\|<\infty$ and the function
$\vec{J}_k(t,\cdot)$ of $\vec{x}\in\Vc$ satisfies $\vec{J}_k(t,\cdot)\in\Hs_\bullet$ for
every $t\in\Tc$.   
...
$\Hs_\bullet$ in \eqref{eq:Helmholtz}, $\vec{\nabla}\chi_k\in\Hs^{\,0}_{\Ds\times}$,
$\|\vec{J}_k\|<\infty$, $\vec{J}_k(t,\cdot)\in\Hs_\bullet$ for every $t\in\Tc$, and
Fubini's theorem \cite{Folland:99}, we have that
$\langle\vec{J}_j\cdot\vec{\nabla}\chi_k\rangle=0$, for all $j,k=1,\ldots,d$. Since 
$\vec{E}_k=\vec{\nabla}\chi_k+\vec{e}_k$, we therefore have
$\langle\vec{J}_j\cdot\vec{e}_k\rangle=\langle\vec{J}_j\cdot\vec{E}_k\rangle$. This is trivially
satisfied in the case of a time-independent velocity field $\vec{u}$,
since in this case $\vec{J}_j\in\Hs_\bullet$ and $\vec{\nabla}\chi_k\in\Hs_\times$.  
This and equation \eqref{eq:sigma*}, implies that the components
...
$\Sb=\Hb-(\Delta^{-1})\Tb$. Consequently, since $\Sb\vec{e}_j=\Hb\vec{e}_j$,
$\langle\Hb\rangle=0$ (component-wise), and $\langle\vec{\nabla}\chi_k\rangle=0$, we have from
\eqref{eq:Eff_Diffusivity} that
%
...
(NOW DERIVE THE RESOLVENT REPRESENTATION FOR $\vec{\nabla}\chi_k$)
