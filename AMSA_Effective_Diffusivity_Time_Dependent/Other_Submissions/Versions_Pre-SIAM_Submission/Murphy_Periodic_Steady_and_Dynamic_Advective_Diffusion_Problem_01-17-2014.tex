...
$\{\Qb_i(\lambda)\}$, $-\infty<\lambda<\infty$, of self-adjoint projection operators - the
...
$\vec{\zeta}=\vec{g}_k$, $\d\mu_{\xi\zeta}(z)$ defined as in equation
...
\begin{align}\label{eq:Int_alpha*_jk}
  \alpha^*_{jk}=\langle F(\Ab)\vec{g}_j\cdot G(\Ab)\vec{g}_k\rangle
     %=\int_I\frac{z\,\d\mu_{jk}(z)}{(\varepsilon+z)\overline{(\varepsilon+z)}}
...
$\mu_{kj}(z)=\overline{\mu}_{jk}(z)$, $\overline{z}=-\I\lambda$, and the
...
implies that $\alpha^*_{kk}=0$, as anticipated.



En route, we showed that the
domain of integration $I$ of the integral representations in
\eqref{eq:Integral_Rep_kappa*} are determined by the spectrum $\Sigma(\Ab)$ of
the operator $\Ab$ and $I\subseteq(-\infty,\infty)$, due to the properties of the
functions $F(z)$, $G(z)$, and $\mu_{jk}(z)$ underlying these
Radon--Stieltjes integral representations. Since the spectral radius
of the normal operator $\Ab$ is given by its norm $\|\Ab\|$
\cite{Reed-1980}, $I$ can be an unbounded set in the case of a
time-dependent velocity field $\vec{u}$ and $I\subseteq[-\|\Ab\|,\|\Ab\|]$ with
$\|\Ab\|\leq\|\Hb\|<\infty$ in the case of a time-independent velocity field. This
concludes our proof of Theorem \ref{thm:Integral_Reps} $\Box$.  
...
of the velocity field $\vec{u}$, which is encapsulated in the
\emph{spectral measure} $\d\bmu$ underlying these integral
representations. In Section \ref{sec:Assymptotics} we discuss in
detail the properties of the spectrum $\Sigma(\Ab)$ of the operator $\Ab$
and show how these properties lead to useful decompositions of
$\d\bmu$. We employ this detailed information regarding the spectral
measure $\d\bmu$ and the properties of the integrals in
\eqref{eq:Integral_Rep_kappa*} to obtain asymptotic behavior of
$\kappa^*_{jk}$ and $\alpha^*_{jk}$ as $\varepsilon\to0$.  
