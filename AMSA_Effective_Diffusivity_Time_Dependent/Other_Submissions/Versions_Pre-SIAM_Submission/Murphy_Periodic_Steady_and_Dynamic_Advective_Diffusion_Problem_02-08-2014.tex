...
$\vec{\xi}=\vec{\nabla}f$  and $\|\Ab\vec{\xi}\,\|=\|Af\|_1$. Moreover, these
operators are related by $\vec{\nabla}A=\Ab\vec{\nabla}$. The Radon--Stieltjes
measures underlying the integral representations of Theorem
\ref{thm:kappa_sigma} and Corollary \ref{cor:Integral_Reps} are
equal, $\d\langle Q_2(\lambda)g_j,g_k\rangle_1=\d\langle\Qb_2(\lambda)\vec{g}_j,\vec{g}_k\rangle$,
$j,k=1,\ldots,d$, up to null sets of measure zero, with
$\vec{\nabla}Q_2(\lambda)=\Qb_2(\lambda)\vec{\nabla}$ (weakly) and $\vec{g}_j=\vec{\nabla}g_j$.         
%
\end{theorem}
%
...
\end{align}
%
and in particular $\vec{\nabla}A^n=\Ab^n\vec{\nabla}$ for all $n\in\mathbb{N}$. 
Consequently, applying the differential opperator $\vec{\nabla}$ to both
sides of the formula $(\varepsilon+A)\chi_j=g_j$ of
\eqref{eq:Resolvent_Rep_Scalar}, we obtain the formula
$(\varepsilon\Ib+\Ab)\vec{\nabla}\chi_j=\vec{g}_j$ of equation
...
$Xf$ is determined by the relations \cite{Stone:64}    
%
...
\end{align}
%
where $h$ is an arbitrary element in $\Fc$. Since $M=-\I A$ is
...
\eqref{eq:X_Q_Correspondence} also holds under the mappings $X\mapsto A$,
...
that for $\Fs$, along with the property $\Fc\sim\Fs$, induce the
one-to-one isometry $D(A)\sim D(\Ab)$. From $\Fc\sim\Fs$, we have for every
$f\in D(A)\subset\Fc$ that $\vec{\nabla}f\in\Fs$, so from equation
\eqref{eq:A_Ab_Relation} 
%
...
$\vec{\nabla}f\in D(\Ab)$. Conversely, from $\Fc\sim\Fs$ we have for every
$\vec{\xi}\in D(\Ab)\subseteq\Fs$ there exists unique $f\in\Fc$ such that
$\vec{\xi}=\vec{\nabla}f$, and equation \eqref{eq:A_Ab_Relation} then implies
that  
%
\begin{align}
...
$f\in D(A)$. In summary, for every $f\in D(A)$ we have $\vec{\nabla}f\in D(\Ab)$ and
$\|Af\|_1=\|\Ab\vec{\nabla}f\|$. Conversely, for each $\vec{\xi}\in D(\Ab)$
there exists unique $f\in D(A)$ such that $\vec{\xi}=\vec{\nabla}f$  and
$\|\Ab\vec{\xi}\,\|=\|Af\|_1$. Consequently, the domains $D(\Ab)$ and $D(A)$
are in one-to-one isometric correspondence, i.e. $D(\Ab)\sim D(A)$.    




From equation \eqref{eq:A_Ab} and the linearity proerties of
...
\end{align}
%
for all $f\in D(A)\iff\vec{\nabla}f\in D(\Ab)$. This implies that
$\vec{\nabla}Q_2(\lambda)=\Qb_2(\lambda)\vec{\nabla}$ (weakly). This, in turn, implies that
the Radon--Stieltjes measures underlying the integral representations
of Theorem \ref{thm:kappa_sigma} and Corollary \ref{cor:Integral_Reps} 
are equal, $\d\langle Q_2(\lambda)g_j,g_k\rangle_1=\d\langle\Qb_2(\lambda)\vec{g}_j,\vec{g}_k\rangle$,
$j,k=1,\ldots,d$, up to null sets of measure zero. This concludes our proof
of Theorem \ref{thm:Formulation_Equivalence} $\Box$.
