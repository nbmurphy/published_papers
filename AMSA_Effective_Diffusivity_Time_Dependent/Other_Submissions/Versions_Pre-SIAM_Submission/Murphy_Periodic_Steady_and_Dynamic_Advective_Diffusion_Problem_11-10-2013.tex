...
\begin{align}\label{eq:ADE_Divergence}
  \partial_tT%&=\varepsilon\Delta T+\vec{u}\cdot\vec{\nabla}T\\
...
the Laplacian $\Delta$. The formulas in \eqref{eq:Maxwells_Equations} are
...
respectively, and $\bsig$ is the local conductivity of the medium. In
the ACM for treating the effective transport properties of composites,
the effective conductivity tensor $\bsig^*$ is defined by 
%
...
$(\Delta^{-1}\partial_t\Ib+\Hb)$, the intensity-flux relationship in
\eqref{eq:Maxwells_Equations} is similar to
that of a Hall medium \cite{,}. Moreover, the skew-symmetry of this
operator implies that
$\langle(\Delta^{-1}\partial_t\Ib+\Hb)\vec{E}_k\cdot\vec{E}_k\rangle=0$. Consequently by equation  
\eqref{eq:Maxwells_Equations} and the Helmholtz theorem,
...
\end{align}
%
We will discuss these concepts in more detail in Section
\ref{sec:Integral_Rep}, where we adapt the ACM to provide a Stieltjes
integral representation for $\bkappa^*$, which holds for both for
