...
Section \ref{sec:Integral_Rep} a Stieltjes integral representation for
...
$\chi=\chi(t,\vec{x}\,;\vec{e}_j)$, $t\in\Tc$, $\vec{x}\in\Vc$, and the bounded
...
however, in this case $\partial_t\chi=0$. In equation \eqref{eq:Cell_Problem},
...
$\Tc\otimes\Vc$. The components $\kappa^*_{jk}=\bkappa^*\vec{e}_j\cdot\vec{e}_k$ of
the effective diffusivity tensor are given in terms of the
...
transport of the tracer $\phi$ is always inhanced by the presence of a
incompressible velocity field. In Section \ref{sec:ACM},  we recast
equations \eqref{eq:Cell_Problem} and \eqref{eq:Eff_Diffusivity} into
a form which parallels the problem of characterizing effective
transport in composite media \cite{Golden:CMP-473}. 
...
formulation of the effective parameter problem and provides a
Stieltjes integral representation for the effective transport
coefficients of composite media, involving a \emph{spectral measure}
of a self-adjoint linear operator which depends only on the composite
geometry \cite{Golden:CMP-473,Murphy:JMP:063506,MILTON:2002:TC}. Here,
we adapt this Hilbert space formulation to treat the problem of
effective transport by advection enhanced diffusion. In Section
...
diffusion by incompressible velocity fields. This abstract Hilbert
space framework is the topic of Section \ref{sec:Hilbert_Space}.      
...
%
In this section, we discuss an abstract Hilbert space formulation of
the effective parameter problem for advection enhanced diffusive
transport by incompressible velocity fields. Consider the Hilbert
spaces $\Hs_{\Tc}$ and $\Hs_{\Vc}$ (over the complex field
...
\end{align}
%
where we have defined $\vec{V}=(V_1,\ldots,V_d)$ and for notational
...
$\Hs_0$ vector fields, with associated orthogonal projectors
$\bGamma_\times=\vec{\nabla}(\Delta^{-1})\vec{\nabla}\cdot$,
...
\begin{align}\label{eq:Helmholtz}
  \Hs_{\Vc}&=\Hs_\times\oplus\Hs_\bullet\oplus\Hs_0,\qquad
...
%we have $|\partial_t\chi|<\infty$ for all $(t,\vec{x})\in\Tc\otimes\Vc$.
...
$\vec{\nabla}\chi\in\otimes_{i=1}^dL^2(\Tc\otimes\Vc)$. Hence,
...
assume that the flow matrix $\Hb$ is bounded in the operator norm
...
leads to a Stieltjes integral representation for $\bkappa^*$, we first
...
$\vec{\nabla}\chi\in\mathscr{H}_\times$, we have $\langle\vec{J}_k\cdot\vec{\nabla}\chi\rangle=0$, where 
...
\cite{McOwen:2003:PDE} provides the existance and uniqueness of a
solution $\vec{\nabla}\chi\in\Hs$ satisfying the cell problem
...
\begin{align}\label{eq:Variational}
  \langle\bsig(\vec{\nabla}\chi+\vec{e}_k)\cdot\vec{\xi}\,\rangle=0, \text{ for all }
  \vec{\xi}\in\Hs.
\end{align}
%
...
\cite{McOwen:2003:PDE} provides the existance of a unique
$\vec{\nabla}\chi\in\Hs$ satisfying \eqref{eq:Cell_Problem} in this
...
prove the existance and uniqueness of a solution $\vec{\nabla}\chi\in\Hs$ 
...
dynamic velocity fields $\vec{u}$, which stem from the unboundenness
...
measure underlying a Stieltjes integral representation of $\bkappa^*$.     
...
\ref{sec:Hilbert_Space}, and the spectral theorem
\cite{Reed-1980,Stone:64,Stakgold:BVP:2000}, to provide a Stieltjes
...
\begin{align}\label{eq:Resolvent_Rep}
  \vec{\nabla}\chi=\I(-\I\varepsilon\Ib-\Mb)^{-1}[\bGamma\Hb\vec{e}_k], \quad
...
\eqref{eq:Bounded_H}, $\Hb$ is also bounded in oparator
...
\end{align}
%
which are the entire space $\Hs$, i.e. $D(\Mb)=D(\Mb^*)=\Hs$. Since
$\Hs$ is a Hilbert space, the sets $D(\Mb)$ and $D(\Mb^*)$ are
closed and the operators $\Mb$ and $\Mb^*$ are consequently
examples of \emph{closed} linear operators
\cite{Reed-1980,Stone:64}. In Section \ref{sec:Appendix} we show that
$\Mb$ is symmetric  
%
...
domain, the operator $\Delta^{-1}$ is a compact \cite{Stakgold:BVP:2000},
hence bounded operator on $\Hs$ (more precisely $\Hs_{\Vc}$). The
operator $\partial_t\Ib$, on the other hand, is \emph{unbounded} on $\Hs$
...
$\{\vec{\psi}_n\}\subset\Hs$, $\langle\vec{\psi}_n\cdot\vec{\psi}_m\rangle=\delta_{nm}$, $n,m\in\mathbb{N}$,
where the  $j^{\,\text{th}}$ component $(\vec{\psi}_n)_j$, $j=1,\ldots,d$, of 
$\vec{\psi}_n$ is given by $(\vec{\psi}_n)_j(t,\vec{x})=a\sin((n+j)\pi t/T)$, where
$a=\sqrt{2/Td\,}$ is the normalization constant. The action of $\partial_t\Ib$
on the members of the set $\{\vec{\psi}_n\}$ is given by $\{\partial_t\vec{\psi}_n\}$
with $(\partial_t\vec{\psi}_n)_j(t,\vec{x})=b_{nj}\cos((n+j)\pi t/T)$, where
$b_{nj}=(n+j)a\pi/T$. The $\Hs$-norm of $\partial_t\vec{\psi}_n$ is given by
$\|\partial_t\vec{\psi}_n\|^2=(T/2)\sum_jb_{nj}^2=\sum_j[(n+j)\pi/T]^2$. This 
clearly demonstrates the unboundedness of the operator $\partial_t\Ib$ on
$\Hs$.
...
extended to a closed linear operator \cite{Reed-1980,Stone:64}.
...
$\Tb$ in place of $\Mb$. However, in general \cite{Reed-1980}, the
domain $D(\Tb^*)$ of its adjoint $\Tb^*$ does not satisfy the property
displayed in equation \eqref{eq:Domain_M} and in such circumsances,
$\Tb$ is \emph{not} self-adjoint on $D(\Tb)$. Only for self-adjoint
linear operators does the spectral theorem hold \cite{Reed-1980}. It
is therefore necesary that we find a domain $D(\Mb)$ for which $\Mb$
is a self-adjoint operator, in order to provide the promised Stieltjes 
integral representation for $\bkappa^*$, involving a spectral measure
of $\Mb$. 
...
functions $\xi\in\hat{\Ds}_{\Tc}$ such that $\xi(0)=\xi(T)$, i.e. functions $\xi$ 
satisfying the properties of equation \eqref{eq:AC_L2} with
$\int_0^Tg(\tau)d\tau=0$. Finally, consider the set $\Ds_{\Tc}$ of all
functions $\xi\in\hat{\Ds}_{\Tc}$ such that $\xi(0)=\xi(T)=0$, i.e. functions $\xi$
satisfying the properties of equation \eqref{eq:AC_L2} with $c=0$ and
$\int_0^Tg(\tau)d\tau=0$. These sets satisfy
$\Ds_{\Tc}\subset\tilde{\Ds}_{\Tc}\subset\hat{\Ds}_{\Tc}$ and are each everywhere dense in
$L^2(\Tc)$ \cite{Stone:64}. Let the operators $\hat{\Ab}$, $\tilde{\Ab}$,
and $\Ab$ be identified as $\I\partial_t$ with domains 
...
\eqref{eq:Resolvent_Rep} is self-adjoint on the space
%
\begin{align}\label{eq:Ht}
  \Hs_t=\{\vec{\xi}\in\As_{\Tc}\otimes\Hs_\times: \langle\vec{\xi}\,\rangle=0\}.
\end{align}
%



We are now ready to provide a Stieltjes integral representation for 
the effective conductivity tensor $\kappa^*$. This follows from the
...
a family $\{Q(\lambda)\}$ - the resolution of the identity - satisfying
$\lim_{\lambda\to-\infty}Q(\lambda)=0$ and $\lim_{\lambda\to\infty}Q(\lambda)$. Moreover, let
$\vec{\xi},\vec{\zeta}\in\Hs_t$ and consider the \emph{measures}
for all \emph{real
  valued} 
...
operator $(\lambda\Ib-\Mb)$ shown in \eqref{eq:Resolvent_Rep} is called the
\emph{resolvent} $\Rb_\lambda=(\lambda\Ib-\Mb)^{-1}$ of the self-adjoint operator
$\Mb$. Using this notation, equation \eqref{eq:Resolvent_Rep} can be
written 
%
\begin{align}\label{eq:Resolvent}
 \vec{\nabla}\chi=\I\Rb_{-\I\varepsilon}\vec{g}_k, \quad
 \vec{g}_k=\bGamma\Hb\vec{e}_k.
\end{align}
%
and is interpreted through operational calculus
%in Hilbert space
\cite{Reed-1980,Stone:64}. The spectral theorem of operational
calculus states that, corresponding to the self-adjoint operator
$\Mb$, there exists a unique family of projection operators $\{Q(\lambda)\}$ -
the resolution of the identity - satisfying $Q(-\infty)=0$ and $Q(\infty)=\Ib$,
such that for ever bounded continuous function
$F:\mathbb{R}\to\mathbb{C}$ and $\vec{\xi},\vec{\zeta}\in\Hs_t$
\cite{Reed-1980,Stone:64}  
%
\begin{align}\label{eq:Spectral_Theorem}
  \langle F(\Mb)\vec{\xi},\vec{\zeta}\rangle = \int_{-\infty}^\infty F(\lambda) \mu(d\lambda), \quad
  \mu(d\lambda)=\langle Q(d\lambda)\vec{\xi},\vec{\zeta}\rangle.
\end{align}
%
Here, $Q(d\lambda)$ is the (unique) projection valued measure associated
with $Q(\lambda)$ and $\mu$ is a spectral measure associated with the
self-adjoint operator $\Mb$ in the $(\vec{\xi},\vec{\zeta})$ state. Another
key result of operational calculus is that, since $\Mb$ is
self-adjoint on $\Hs_t$, $\Rb_\lambda$ is also self-adjoint on
$\Hs_t$. Therefore, from equation 
