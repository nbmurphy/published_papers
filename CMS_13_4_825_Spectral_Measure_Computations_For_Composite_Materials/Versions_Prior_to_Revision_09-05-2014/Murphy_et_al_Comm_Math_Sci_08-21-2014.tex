...
equation \eqref{eq:Component_sigma_def} and using the spectral theorem
for the symmetric operator $\chi_1\Gamma\chi_1$ displayed in equations
\eqref{eq:Spectral_Theorem}, \eqref{eq:Discrete_Spectral_Theorem}, and 
...
This demonstrates that, as in the formulation of the ACM given in
Section \ref{sec:Continuum_Setting}, the natural Hilbert space
underlying the integral representation in equation
...
method provides the Stieltjes integral representations for the
effective transport coefficients of such composite media, displayed in
equation \eqref{eq:Stieltjes_F}, which involve spectral measures associated
with the self-adjoint random operators $M_i=\chi_i\Gamma\chi_i$ and
...
turbulent flows, the effective complex permittivity for uniaxial
...
$\mathscr{H}_0=\mathscr{H}_\times\cup\mathbb{R}^d$ by 
%
\begin{align}
  \mathscr{H}_0=
...
with the constant fields $\mathbb{R}^d$ included. Equip $\mathscr{H}_0$ with the
...
self-adjoint operator $M_1$ is real and the spectral radius of $M_1$ is equal to
...
positive operator and therefore has spectrum $\Sigma\subseteq[0,\infty)$
\cite{Stone:64}.  Consequently, the spectrum $\Sigma$ of $M_1$ is real and
...
the functional has the following integral representation involving the
...
$d$-dimensional bond lattice $\mathbb{Z}_L^d$ onto the one dimensional
set $\mathbb{N}_L$. Moreover, we showed that, under the
mapping $\Theta$, the random operator $M_1=\chi_1\Gamma\chi_1$ can be represented by a
random matrix of size $N\times N$, where $N=d\,L^d$,
...
induced by the dot-product on $\mathbb{R}^N$
\cite{Demmel:1997}. Furthermore, $\chi_1$ is a \emph{random}, diagonal
projection matrix satisfying $\chi_1^2=\chi_1$, with zeros and ones along
its diagonal. Consequently, $\chi_1$ is real-symmetric and satisfies $\|\chi_1\|\leq1$.
...
$\vec{\xi}\in\mathbb{R}^N$ we have that
...
$i=1,\ldots,N$, of the symmetric matrix $M_1$ form an orthonormal basis for
...
$f:\mathbb{C}\mapsto\mathbb{C}$.  Consequently, for all
$\vec{\xi},\vec{\zeta}\in\mathbb{R}^N$, the functional
