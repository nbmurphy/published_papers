...
provided a novel formulation of the ACM, which holds for both this
...
effective transport coefficients of composites with finite lattice
microstructure, and demonstrates that the underlying spectral measures
are given explicitely in terms of the eigenvalues and eigenvectors of
the random matrices.


In this section, we utalize the mathematical framework described above
to directly compute the spectral measures and effective transport
coefficients for such composite media. In particular, following the
statement of Theorem \ref{thm:Discrete_Spectral_Theorem_ACM}, we
introduced a large family of random bond lattices, with various types
of isotropic and anisotropic composite microstructures.  In Section 
\ref{sec:Finite_Lattice_Setting} we introduced a numerically efficient
projection method, summarized by equations
\eqref{Pi_coordinates_E}--\eqref{eq:Fs_U1}, to facilitate the
computation of the spectral measures and effective transport
coefficients of such composite media. In this section, we employ this 
projection method to directly compute the spectral measures and
effective parameters associated with these composite microstructures. 
