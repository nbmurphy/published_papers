...
to enable such computations. This is accomplished by providing a novel
formulation of the analytic continuation method which is equivalent to
the original formulation and holds for both the finite lattice setting
and the infinite settings. We also introduce a
family of random bond lattices and directly compute the associated spectral
...
This, the property
$[\chi_1U]^T\hat{e}_j\,\cdot\,U^T\hat{e}_k=[\chi_1U]^T\hat{e}_j\,\cdot\,[\chi_1U]^T\hat{e}_k$,
and equation \eqref{eq:Matrix_Functional_proof} yield
%
\begin{align}\label{eq:Stieltjes_F_DiscretE}
...
\end{align}
%
From equation \eqref{eq:Matrix_Rep_Spec_Theorem} we have 
...
are zero for $i=1,\ldots,N_0$.  Let $\vec{u}_m^{\,1}$ and $\lambda_m^1$,
$m=1,\ldots,N_1$, denote the eigenvalues and eigenvectors of the $N_1\times N_1$
...
\begin{align}\label{eq:Fs_U1}
  F_{jk}(s)=\langle(sI_1-\Lambda_1)^{-1}[U_1^T\hat{e}_j^{\,\pi_1}]
          ...
          =\left\langle\sum_{m=1}^{N_1} 
          \frac{
                (\vec{u}_m^{\,1}\cdot \hat{e}_j^{\,\pi_1})
               (\vec{u}_m^{\,1}\cdot \hat{e}_k^{\,\pi_1})
              }{s-\lambda_m^1}
              \right\rangle.          
\end{align}
%
...
associated with a large family of random bond lattices, introduced in
Section \ref{sec:Finite_Lattice_Setting}. In particular, we 
developed in Section \ref{sec:Theorem_Proof} a numerically
...
the diagonal components of the effective transport tensors and the
underlying spectral measures, e.g., $\sigma^*_{kk}=\sigma_2m_{kk}(h)$ and
$\mu_{kk}$, for $k=1,\ldots,d$.   
...
$M_1(\omega)=\chi_1(\omega)\,\Gamma\chi_1(\omega)$, for $\omega\in\Omega$. In particular, for $\omega\in\Omega$
fixed, the measure  $\mu_{kk}(\omega)$ is a weighted sum of $\delta$-measures centered at the
eigenvalues $\lambda_i(\omega)$ of $M_1(\omega)$, $i=1,\ldots,N$, with spectral weights
...
\eqref{eq:Projection_Eigenspace} implies that the spectral weights are
identically zero for $i=1,\ldots,N_0(\omega)$. This was used in equation
\eqref{eq:Fs_U1} to show that the measure $\mu_{kk}(\omega)$ depends only on the
eigenvalues $\lambda^1_i(\omega)$ and eigenvectors $\vec{u}^{\,1}_i(\omega)$ of the
principle sub-matrix $\Gamma_1(\omega)$ of $\Pi(\omega) M_1(\omega)\Pi^T(\omega)$, introduced in
equation \eqref{eq:Spec_Decomp_chi_Gamma_chi_Proof}. Consequently, for 
fixed $s\in\mathbb{C}\backslash[0,1]$, the value of
the effective complex conductivity $\sigma^*_{kk}=\sigma_2(1-F_{kk}(s))$ of
the medium can be obtained by computing the eigenvalues
$\lambda^1_i(\omega)$ and eigenvectors $\vec{u}^{\,1}_i(\omega)$ of the matrix
$\Gamma_1(\omega)$, for each  $\omega\in\Omega$. This projection method makes the numerical
computation of the spectral measure $\mu_{kk}$ more efficient,
...
$d=3$ and typically used $|\Omega_0|\sim10^5$ geometric configuratons of the
random lattices. In this case, the variance $\text{Var}(\sigma^*_{kk})$ of
...
of the spectral measure $\mu_{kk}$, called the \emph{spectral function}
$\mu_{kk}(\lambda)$, begins to resemble a smooth curve, as shown in Figure
\ref{fig:Anisotropic_Spectral_Measures}. The spectral function
$\mu_{kk}(\lambda)$ (not to be confused with the function
$\langle Q(\lambda\hat{e},\hat{e}\rangle_1$) is determined as follows. First, the
spectral interval $[0,1]$ is divided into $M\sim10^2$ sub-intervals
$I_m$, $m=1,\ldots,M$, of equal length $1/M$. Then all the eigenvalues
$\lambda^{\,1}_i(\omega)\in I_m$, for $i=1,\ldots,N_1(\omega)$ and $\omega\in\Omega_0$ are isolated. The
height of the spectral function $\mu$     
