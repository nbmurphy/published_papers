...
measures, e.g., $\sigma^*_{kk}=\sigma_2m_{kk}(h)$ and $\mu_{kk}$, $k=1,\ldots,d$. In
...
$\vec{u}^{\,1}_i(\omega)$, for each $\omega\in\Omega$. Since the computational cost of
finding all the eigenvalues and eigenvectors of a $N\times N$ matrix is 
$O(N^3)$ \cite{Demmel:1997}, this ``projection method'' makes the
numerical computation of $\mu_{kk}$ and $\sigma^*_{kk}$ much more efficient,
...
\caption{Spectral measures and effective complex conductivities for
  ...
  displayed in (b) and the effective complex conductivity for
  $s\approx-0.034+\imath\,0.032$ is displayed $\sigma^*_{11}$ in (c), along with the
  corresponding isotropic bounds. The computed spectral functions have
...
$s=1/(1-\sigma_1/\sigma_2)$. Consistent with the symmetries of
...
for $p_1^1=p_1^2$ (to numerical accuracy and statistical truncation).
...
with $L=60$. Statistical realizations of the composite microstrucure
...
associated spectral functions displayed in Figure
...
local geometric structures called ``fractal animals.''  In Figure 
\ref{fig:LocIsotropic_RRN_11}(c), the corresponding values of the
effective complex conductivities $\sigma^*_{11}$ and $\sigma^*_{22}$ are
displayed along with the isotropic bounds from equation
...
$\mu_{11}(\lambda)$ shown in Figure
\ref{fig:LocIsotropic_RRN_11}(b). Consequently, $\sigma^*_{11}=\sigma^*_{22}$ to
numerical accuracy and statistical truncation. The spectral functions
...
similar to that in Figure \ref{fig:LocIsotropic_RRN_11}(b) for locally 
isotropic random media. Moreover, the spectral functions 
$\kappa_{kk}(\lambda)$, $k=1,2$, associated with the effective resistivity $\rho^*_{kk}$,
for 2D locally and statistically isotropic random media, also look quite
similar to $\mu_{11}(\lambda)$ in Figure \ref{fig:LocIsotropic_RRN_11}(b). 
...
difference between the theory and the numerical calculation seen in
Figure \ref{fig:Duality_RRN_11}(b) is reduced as $L$ increases, and is
virtually absent for $L=100$. In Figure \ref{fig:Duality_RRN_11}(c),
...
\eqref{eq:Isotropic_Bounds}, respectively, for
$s\approx-0.034+\imath\,0.032$. The computed spectral functions and effective
parameters are in excellent agreement with the theoretical duality
prediction, which holds for \emph{infinite} systems. The deviation in
the computed values of the effective parameters relative to the
duality prediction is typically $\lesssim10^{-2}$ for $L=60$ and decreases 
as $L$ increases. 
