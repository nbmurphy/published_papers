...
\caption{Spectral measures and effective complex conductivities for
  ...
  displayed in (b). The values of the effective complex conductivity
  $\sigma^*_{kk}$ and resistivity $\rho^*_{kk}$, $k=1,2$, are displayed in (c)
  and (d), respectively, along with the corresponding isotropic bounds
  for   $s\approx-0.034+\imath\,0.032$. The computed spectral functions have been
...
One challange is the numerical cost of computing \emph{all} of the
eigenvalues and eigenvectors of a $N\times N$ real-symmetric matrix, which
is $O(N^3)$ \cite{Demmel:1997}. However, in the case of the 2D
statistically self dual random lattices discussed above, the deviation
in the computed values of the effective parameters for $L=15$,
relative to the theoretical duality prediction for the \emph{infinite
  lattice}, is typically $\lessapprox0.3$. This indicates that the computations
of the effective transport coefficients are reasonably accurate even
...
Another challange associated with a large matrix size $N$, is the
...
\emph{integer elements}, the matrices $\Gamma$ and $\Upsilon$ were effeciently
computed using MATLAB's sparse arctecture, which also reduces roundoff
...
problem for the matrices $M_1=\chi_1\Gamma\chi_1$ and $K_1=\chi_1\Upsilon\chi_1$ is determined
...
We now discus the condition numbers of the matrices $\Delta$ and $C^TC$ for
the system sizes considered. Namely, $L=60$ and $100$ for $d=2$, and
$L=10$ and $15$ for $d=3$. Recall that for $d=2$, $C^TC=\nabla^T\nabla=\Delta$. In
this 2D case, $\mathcal{K}(\Delta)\approx1.6\times10^3$ for $L=60$ and
$\mathcal{K}(\Delta)\approx4.4\times10^3$ for $L=100$. For $d=3$, $C^TC\neq\Delta$. In this 3D
case, $\mathcal{K}(\Delta)\approx51$ for $L=10$ and $\mathcal{K}(\Delta)\approx109$ for
$L=15$. However, $\mathcal{K}(C^TC)\approx6.4\times10^5$ for $L=10$ and
$\mathcal{K}(C^TC)\approx7.4\times10^6$ for $L=15$. These condition numbers were  
...
$\approx1$ for the system sizes considered, indicating that the eigenvalue
problems associated with these matrices are well-conditioned. In
summary, within the double precision archtecture of MATLAB with a
\emph{machine epsilon} $\epsilon\approx2.2\times10^{-16}$, for the system sizes $L$
considered, the spectral measure computations associated with the
matrices $M_1=\chi_1\Gamma\chi_1$ and $K_1=\chi_1\Upsilon\chi_1$ are well-conditioned for
$d=2$. The spectral measure computations associated with the matrix
$M_1=\chi_1\Gamma\chi_1$ are also well-conditioned for $d=3$. While the spectral
measure computations associated with the matrix $K_1=\chi_1\Upsilon\chi_1$ are
relatively ill-conditioned for $d=3$. The problem of finding an
appropriate \emph{preconditioner} for the matrix $C^TC$ in 3D is a
...
\caption{Spectral measures for 3D locally isotropic random media. The
  spectral function $\mu_{11}(\lambda)$ is displayed in (a) for various volume
...
Displayed in Figure \ref{fig:3D_Spectral_Measure_Curl} are our
computations of the spectral functions $\kappa_{11}(\lambda)$ associated with the
effective complex resistivity $\rho^*_{11}$, for 3D
locally isotropic random media with $L=10$ and various values of
$p_1$. The spectral functions $\kappa_{11}(\lambda)$ displayed in  
Figure \ref{fig:3D_Spectral_Measure_Curl} have a transitional
...
in the computation of $\Upsilon=C(C^TC)^{-1}C^T$, which is then propogated to
the eigenvalue problem for $K_1=\chi_1\Upsilon\chi_1$, has a smoothing effect, and
there is no resonant structure of the spectral functions for small
values of $p_1$. This smoothing effect is also typical of ill-posed
inverse problems for the reconstruction of spectral measures
\cite{}. Consisten with isotropy, $\kappa_{11}(\lambda)\approx\kappa_{33}(\lambda)$ and
$\rho^*_{11}\approx\rho^*_{33}$. Although, the behavior of $\kappa_{22}(\lambda)$ and the
value of $\rho^*_{22}$ is significantly different from the other two
components. 
...
open bonds first appears. For the two-dimensional statistically
isotropic lattice percolation
...
$p_2\to p_c^-$. Since $s=1/(1-h)$ and $t=1/(1-z)$, where $z=1/h$, we see
from equation \eqref{eq:Stieltjes_F} that the associated critical
behavior of the integral representations of
...
Consider the behavior of the spectral measure at the spectral
...
indicate that the vanishing of the spectral gaps lead to a buildup in
the mass of the measure at $\lambda=0$, while the mass of the measure is
approximately zero for $\lambda=1$, i.e., $\mu_{11}(1)\approx0$. Moreover, as $p_1$
increases beyond the percolation threshold $p_c$, the mass of the
measure at $\lambda=1$ remains approximately zero while the buildup of the
mass of measure mass at $\lambda=0$ \emph{persists and grows}.



Now consider the behavior of the spectral measure at the spectral
endpoints $\lambda=0,1$ for the 3D lattice percolation model. In Figure
\ref{fig:3D_Spectral_Measures}(a) we see that, as $p_1$ increases from
zero and approaches the percolation threshold $p_c\approx0.2488$, a gap in
$\mu_{11}(\lambda)$ about $\lambda=0$ shrinks and then vanishes, leading to a
...
to grow, while a gap in the spectral function at $\lambda=1$ shrinks and
then vanishes for $p_1=1-p_c\approx0.7512$, with $\mu_{11}(1)\approx0$. 




Displayed in Figure \ref{fig:Effective_Parameter_Behavior} is the
behavior of the effective conductivity $\sigma^*_{kk}$, $k=1,\ldots,d$,
for $s\approx-0.034+\imath\,0.032$ as a function of volume fraction
$p_1$, for 2D (a) and 3D (b) locally isotropic RRN. The respective
masses of the measures $\mu_{kk}$ concentrated at the spectral endpoint
$\lambda=0$ are displayed in (c) and (d). It can be seen in Figure
...
behavior in the measure at $\lambda=0$ leads to a significant transition in
the value of $\sigma^*_{kk}$ as the volume fraction $p_1$ surpasses
$p_c$. The associated mass of $\mu_{kk}$ concentrated at $\lambda=1$ is
$\sim10^{-30}$ for both 2D and 3D. 
...
$p_1=p_c$ and $p_1=1-p_c$. Recall that the weights $m_{kk}(0)$ and
$w_{kk}(0)$ of the delta components at $\lambda=0$ and $\lambda=1$ in
