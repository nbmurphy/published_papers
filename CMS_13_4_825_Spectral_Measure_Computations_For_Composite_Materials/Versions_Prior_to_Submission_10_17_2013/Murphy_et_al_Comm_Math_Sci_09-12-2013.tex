...
from effective permittivity data for an interval of electromagnetic
frequency which, in turn, can used to recover microstructural
parameters 
\cite{Cherkaeva:IP-1203,Cherkaeva:IP-065008,Zhang:JCP-5390,Bonifasi-Lista:PMB-3063,Cherkaev:JBiomech-345,Day:JPCM-99,Golden:J_Biomech:337}
and calculate the effective thermal conductivity, viscoelastic
modulus, and other effective properties 
\cite{Cherkaeva:IP-1203,Cherkaev:JBiomech-345}. For classes of
composites which 
...
components $\sigma_{jk}(\vec{x},\omega)$, $j,k=1,\ldots,d$, that are (spatially)
...
%
Consequently,
%we have the following energy representations involving
...
\end{align}
%
Here, we have defined the complex variables $s=1/(1-h)$ and
$t=1/(1-z)=1-s$, $I$ is the identity operator on $\mathbb{R}^d$, the
operator $\Gamma=\vec{\nabla}(\Delta^{-1})\vec{\nabla}\cdot$ is based on convolution with the
free-space Green's function for the Laplacian
$\Delta=\vec{\nabla}\cdot\vec{\nabla}=\nabla^{\,2}$, and the operator
$\Upsilon=-\vec{\nabla}\times(\bDelta^{-1})\vec{\nabla}\times$ involves the vector Laplacian
$\bDelta=- \vec{\nabla}\times\vec{\nabla}\times + \vec{\nabla}\vec{\nabla}\cdot $
\cite{Golden:CMP-473,Murphy:JMP:063506}.     
...
specifically, for example, $\mu_{jk}(d\lambda)=\langle R(d\lambda)\vec{e}_j,\vec{e}_k\rangle_1$,
...
$\int_0^1R(d\lambda)=I$. Moreover, recall that the associated operator $R(\lambda)$
...
$P$ is the associated probability measure. Define
...
\cite{Golden:J_Biomech:337,Murphy:JMP:063506}. The matrix
representations of the operators $M_2=\chi_2\Gamma\chi_2$ and $K_2=\chi_2\Upsilon\chi_2$ are
analogously determined with use of the identity $\chi_2(\omega)=I-\chi_1(\omega)$,
where $I$ is the identity matrix on $\mathbb{R}^N$.
...
\end{align}
%
where $R_i=\vec{u}_i\vec{u}_i^{\,T}$. Furthermore, the mass $\mu_{jk}^0$ of the
...
micro-geometries. Namely, the class of geometries such that $N_1^k(\omega)$ 
is a non-random constant $N_1^k$ for all $k=1,\ldots,d$,
...
bonds, one can define a class of highly \emph{anisotropic} composite
geometries by fixing $p_1^k$ close to $p_1$ for some $k=1,\ldots,d$,
...
$p_1^k=p_1/d$ for all $k=1,\ldots,d$ and $\omega\in\Omega$. Consequently, equation
...
with mean $\langle N_1^k(\omega)\rangle=p_1N/d$. (IS THIS PRECISELY TRUE?) 
...
\begin{align}\label{eq:Measure_Mass_Lattice}
 ...
 &\mu_{kk}(d\lambda)=\sum_{i\in\mathbb{N}_L}
         ...
         =\sum_{i\in\mathbb{N}_L}
             \langle\delta_{\lambda_i^{\Pi_1}}(d\lambda)|R_i^{\,\Pi_1}\hat{e}_k^{\,\Pi_1}|^2\rangle,            
\end{align}
%
...
measures. Using equation \eqref{eq:block_diag_chi} we may write
...
$L^d$, respectively, and similarly for the $\vec{e}_j$ for $j=2,\ldots,d$
(see Section \ref{sec:Finite_Lattice_Setting} for details). Since
$\chi_1$ is a symmetric projection matrix, equations
...
the discussion in the paragraph following this equation
the formulas $\mu_{kk}^0=d\,p_1^k$ and $\eta_{kk}^0=d\,p_2^k$ also hold for
the infinite lattice setting with $p_i^k=\lim_{N\to\infty}\langle N_i(\omega)\rangle/N$,
$i=1,2$, and are given by $\mu_{kk}^0=p_1$ and $\eta_{kk}^0=p_2$ for
isotropic random media.  
...
moments $\mu^n$ of the measure $\mu$ for $n=1,\ldots,J$. Define the set
...
continuum and lattice settings. This method provides Stieltjes integral
representations for the effective parameters of two-phase random
media, involving spectral measures associated with the random
operators $M_i=\chi_i\Gamma\chi_i$ and $K_i=\chi_i\Upsilon\chi_i$. Here, $\chi_i$ is the indicator
function for material phase $i=1,2$ and the operators 
...
In Section \ref{sec:Finite_Lattice_Setting} we developed
the ACM for the \emph{finite} lattice setting. We also provided a
projection method, summarized in equations
\eqref{eq:Spec_Decomp_chi_Gamma_chi} and
\eqref{eq:measure_equivalence}, for numerically efficient, rigorous 
computation of spectral measures and effective parameters 
for such composite media. In this case the operators $\chi_i$, $\Gamma$, and
$\Upsilon$ are represented by real-symmetric projection matrices. The
spectral measures of the corresponding matrices $M_i$ and $K_i$ are
given explicitly in terms of their eigenvalues and eigenvectors, as
displayed in equation \eqref{eq:Stieltjes_F_Discrete} of Theorem
\ref{thm:Discrete_Spectral_Theorem_ACM}. In the paragraph following
the statement of Theorem \ref{thm:Discrete_Spectral_Theorem_ACM}, we
introduced three classes of locally isotropic, statistically
isotropic, and anisotropic random media on finite bond lattices. In
this section we employ this projection method to directly calculate
the spectral measures and effective parameters for these composite
microstructures.
...
\caption{Spectral measures and effective conductivities for
  ...
  are displayed in (a). The corresponding spectral functions are
  displayed for $\mu_{11}$ in (b) and $\mu_{22}$ in (c), while the
  effective complex conductivities $\sigma^*_{11}$ and $\sigma^*_{22}$ are
  displayed in (d) along with the associated first-order bounds. The
  computed spectral functions have been rescaled so that the area
  under the curve is the measure mass.
        }
...
%
\begin{figure}[t]
  \centerline{\includegraphics[scale=0.755]{Locally_Isotropic_RRN_11.eps}}
\caption{Spectral measures and effective conductivities for locally 
  isotropic random media. Statistical realizations of the square
  lattice percolation model are displayed in (a) with the
  corresponding spectral function for $\mu_{11}$ in (b) and effective
  complex conductivity $\sigma^*_{11}$ in (c), along with the corresponding
  isotropic bounds. The computed spectral functions have been rescaled
  so that the area under the curve is the measure mass.
        } 
...
$M_1$, for example,  increases and its eigenvalues become increasingly
dense in the spectral interval $[0,1]$. For a large enough fixed
system, or for a 
random system averaged over many statistical realizations, a high
resolution histogram representation of the spectral measure $\mu_{11}$,
called the \emph{spectral function}, begins to resemble a smooth
curve, as shown in Figure \ref{fig:Anisotropic_Spectral_Measures}. In
Figure \ref{fig:Anisotropic_Spectral_Measures}(a) statistical
realizations of the anisotropic 2D bond lattice for $L=60$ are
displayed for a volume (number) fraction $p_1=0.5$ of type-one bonds
and various values of $p_1^k$, $k=1,2$, the number fraction of
type-one bonds in the positive $k^{\text{th}}$ direction. Here, the
type-one and type-two bonds are colored black and white, respectively,
while the largest connected cluster of type-one bonds is colored
grey. In Figure \ref{fig:Anisotropic_Spectral_Measures}(b) and (c) we
display the behavior of the spectral functions for $\mu_{11}$ and
$\mu_{22}$, respectively, as a function of $p_1^k$, $k=1,2$.  Consistent
with the symmetries of the model, these spectral functions satify 
$\mu_{11}=\mu_{22}$ for $p_1^1=p_1^2$. In Figure
\ref{fig:Anisotropic_Spectral_Measures}(c) the computed values of the
effective complex conductivity $\sigma^*_{11}$ and $\sigma^*_{22}$ are 
displayed, and also satisty $\sigma^*_{11}=\sigma^*_{22}$ for $p_1^1=p_1^2$. 
Also displayed in Figure
\ref{fig:Anisotropic_Spectral_Measures}(c) are the first order bounds
of equation \eqref{eq:0th_order_Bounds}, which 
depend only on the mass $\mu^0_{kk}=d\,p_1^k$ of the measure $\mu_{kk}$ and the
contrast parameter $s=1/(1-\sigma_1/\sigma_2)$. The values of the component
conductivities $\sigma_1$ and $\sigma_2$ were take to be that of the brine and
pure ice phase, respectively, for a sample of sea ice measured at 
a frequency of 4.75 GHz \cite{Backstrom:2007:Book}:
$\sigma_1=51.0741+\I\,45.1602$ and $\sigma_2=3.07+\I\,0.0019$.
%
%
\begin{figure}[t]
  \centerline{\includegraphics[scale=0.7]{A_Duality_RRN_11.eps}}
\caption{Self duality and the percolation transition. Realizations of
  the two-dimensional lattice percolation model (a) are displayed with
  the corresponding spectral function $\mu_{11}$ in (b), and effective
  complex conductivity  $\sigma^*_{11}$ or resistivity $\rho^*_{11}$ in
  (c). In (b) the theoretical prediction for self dual composite
  microstructures is also displayed. In (c) the behavior of the
  effective complex conductivity or resistivity is displayed along
  with the self duality theoretical prediction, and the first-order
  and isotropic bounds. The computed spectral functions have been
  rescaled so that the area under the curve is the measure mass.              
        }
...
We now consider the isotropic composite classes which were introduced
...
spectral function for $\mu_{11}$ and the effective complex conductivity
...
realizations of the percolation model are displayed, with the same
bond color scheme as that in Figure
\ref{fig:Anisotropic_Spectral_Measures}(a). The spectral functions
displayed in Figure \ref{fig:LocIsotropic_RRN_11}(b) have been rescaled 
so that the area under the curve is the mass of the measure. These
...
isotropy, the behavior of $\mu_{22}$ is very similar to that of $\mu_{11}$
in Figure \ref{fig:LocIsotropic_RRN_11}(c). The spectral
functions for $\mu_{kk}$, $k=1,2$, in the case of statistically
isotropic random media were computed in \cite{Murphy:JMP:063506}. They
look very similar to that for the locally isotropic case displayed in
Figure \ref{fig:LocIsotropic_RRN_11}(c). Moreover, the spectral functions
for $\kappa_{kk}$, $k=1,2$, underlying the effective resistivity $\rho^*_{kk}$,
for locally and statistically isotropic random media also look quite
similar to that for $\mu_{11}$ in Figure
\ref{fig:LocIsotropic_RRN_11}(c).
...
isotropic composite microstrucrues for $p_1=0.5$ are statistically
self-dual \cite{MILTON:2002:TC} (note that the anisotropic case
for $p_1^1=p_1/d$ is equivalent to the statistically isotropic
case). For such systems, the spectral    
measures and effective parameters may be explicitly calculated
\cite{MILTON:2002:TC}, e.g. $\mu_{11}(\lambda)=(\sqrt{(1-\lambda)/\lambda}\;)/\pi$
and $\sigma^*_{11}=\sqrt{\sigma_1\sigma_2}$. In Figure \ref{fig:Duality_RRN_11} the
computed spectral measures and effective parameters are displayed for
such random media in the \emph{finite} lattice setting for $L=60$, and
compared with the self-duality theoretical predictions for the
...
Figure \ref{fig:Duality_RRN_11}(b), the computed spectral measures are 
displayed along with the theoretical prediction. The computed spectral
functions have been rescaled so that the area under the curve is the
mass of the measure. In Figure \ref{fig:Duality_RRN_11}(c) the
computed effective parameters are displayed with the theoretical
predictions, and the first-order and isotropic bounds of equations 
\eqref{eq:0th_order_Bounds} and \eqref{eq:Isotropic_Bounds},
respectively, with the same component conductivities as that in Figure
\ref{fig:LocIsotropic_RRN_11}(c). The computed spectral measures and
effective parameters are in excellent agreement with the theoretical
predictions. The error in the computed values of the effective
parameters, relative to the theoretical prediction, is of order
$10^{-2}$ for $L=60$ and decreases as $L$ increases.
...
\caption{Behavior of the effective complex conductivity tensor
  $\bsig^*$ and the endpoint masses of the spectral measure
  $\bmu$ for 2D and 3D locally isotropic random resistor network
  (RRN). The behavior of the diagonal components of $\bsig^*$, as a
  function of volume fraction $p_1$, for 2D and 3D RRN is displayed
  for component conductivities $\sigma_1=51.0741+\I\,45.1602$ and
  $\sigma_2=3.07+\I\,0.0019$ in (a) and (e), and $\sigma_1=63.3+\I\,1930$ and
  $\sigma_2=3.06$ in (b) and (f). The masses of the diagonal components of
  $\bmu$ at the spectral endpoints $\lambda=0,1$ correspoinding to 2D RRN
  are given in (c) and (d), while that for 3D RRN are given in (g) and
  (h). 
        }
...
We now discuss the gap behavior of the spectral measures in terms of
percolation models for two-phase random media in the lattice
setting. In the infinite setting, the isotropic composite
microstructues discussed in this section are examples of percolation
models, which depend only on the volume fraction $p_1=1-p_2$ of the
constituents, hence $m(h)=m(p_1,h)$, for example. In these percolation
models \cite{Stauffer-92,Torquato:RHM-02}, the bonds are 
...
appears. Now consider transport through the associated \emph{random
  resistor network} (RRN), where the bonds are assigned electrical
conductivities $\sigma_1$ with probability $p_1$ and $\sigma_2$ with probability
$p_2$. The effective conductivity $\sigma^*(p_1,h)$, for example, exhibits
two types of critical behavior as $h=\sigma_1/\sigma_2\to0$. First, when $\sigma_1\to0$
and $0<\sigma_2<\infty$, $\sigma^*=0$ for $p_1>p_c$ while $\sigma^*>0$ for $p_1<p_c\,$.
...
In Figures \ref{fig:LocIsotropic_RRN_11}(c) and
\ref{fig:Duality_RRN_11}(c) we see that, as $p_1$ increases from zero
and the system becomes increasingly connected, gaps in the measure
$\mu_{11}$ at the spectral endpoints $\lambda=0,1$ shrink and then vanish 
symmetrically at a value of $p_1=p_c=0.5$. Figure
\ref{fig:Duality_RRN_11}(c) shows that the vanishing of the gaps in
the spectral function lead to a buildup in the mass of the measure at
$\lambda=0$, while the mass of the measure is approximately zero for
$\lambda=1$. In Figure \ref{fig:3D_Spectral_Measures}(a) and (b) we display
the behavior of the spectral functions for $\mu_{11}$ and $\kappa_{11}$,
as a function of $p_1$, for locally isotropic 3D RRN. Like its
2D counterpart, the specral function for $\mu_{11}$ has a rich resonant
structure. Furthermore, as $p_1$ increases from zero and approaches
the percolation threshold $p_c\approx0.2488$, a gap in the spectral function
at the left spectral endpoint $\lambda=0$ shrinks and then vanishes,
leading to a buildup in the mass of the mearure at $\lambda=0$ for
$p_1=p_c$. As $p_1$ increases beyond $p_c$, the eigenvalues continue
to pile up at $\lambda=0$, while a gap in the spectral function at $\lambda=1$
shrinks and then vanishes for $p_1=1-p_c$. The spectral measures for
statistically isotropic 3D RRN  were also computed in
\cite{Murphy:JMP:063506} and have a very similar behavior to that for
locally isotropic RRN displayed in Figure
\ref{fig:3D_Spectral_Measures}. The behavior of the spectral funtion
for $\kappa_{11}$ has a similar gap behavior and subsequent build up at the
spectral endpoints for $p_1=p_c$ and $p_1=1-p_c$.





The behavior of the spectral measures $\mu_{kk}$, $k=1,\ldots,d$, at the
spectral enpoints $\lambda=0,1$ and the behavior of the effective complex
conductivity $\sigma^*_{kk}$, as a function of volume fraction $p_1$, for
2D and 3D locally isotropic RRN is displayed in Figure
\ref{fig:Effective_Parameter_Behavior}. The behavior of $\sigma^*_{kk}$
displayed in Figure \ref{fig:Effective_Parameter_Behavior}(a) and (e)
corresponds to component conductivities 
$\sigma_1=51.0741+\I\,45.1602$ and $\sigma_2=3.07+\I\,0.0019$ which are that of
the brine and pure ice phase, respectively, for a sample of sea ice
measured at a frequency of 4.75 GHz \cite{Backstrom:2007:Book},
yielding a value of the contrast parameter $s=1/(1-\sigma_1/\sigma_2)\approx-0.034+
\I0.032$. The behavior of $\sigma^*_{kk}$ in (b) and (f) correspond to component
conductivities $\sigma_1=63.3+\I\,1930$ and $\sigma_2=3.06$, which are that of
brine and pure ice measured at a frequency of 4.75 GHz
\cite{Backstrom:2007:Book}, yielding a value of the contrast parameter
$s\approx-5\times10^{-5}+ \I0.002$. From this figure we see that the value of the
contrast parameter plays a key role in the behavior of the effective
complex coductivity, as a function of $p_1$. Figure
...
concentrated at the spectral endpoints $\lambda=0,1$. This leads to
significant changes in $\sigma^*_{kk}$ as $p_1$ surpasses $p_c$, as can be
seen in this figure. Please notice the scale of the axes. 


%
\begin{figure}[]
  \centerline{\includegraphics[scale=0.7]{Spectral_Function_Symmetries.eps}} 
\caption{Spectral function symmetries. Transformations of the computed
  ...
  spectral functions have \emph{not} been rescaled to make the area
  under the curve the measure mass.  
        }
...
random media \cite{Murphy:JMP:063506}, and consequently holds for
percolation models of such media. This equation characterizes the 
percolation transition with the formation of delta components in the
spectral measures at the spectral endpoints $\lambda=0,1$, \emph{precisely} at
$p_1=p_c$ and $p_1=1-p_c$. For example, when the function
$m_{kk}(0)=m_{kk}(p_1,0)$, $k=1,\ldots,d$, increases from zero for
$p_1\lesssim p_c$ and $\sigma_1=0$, or when $w_{kk}(0)=w_{kk}(p_2,0)$ increases
from zero for $p_2\gtrsim1-p_c$ and $\sigma_2=0$, the masses of the delta
components in the measures at $\lambda=0$ and $\lambda=1$ increase from zero, 
respectively.
%, leading to critical behavior in the efective conductivity.
Equation \eqref{eq:Measure_Relations} also provides a
relationship between the measures $\mu_{jk}$ and $\alpha_{jk}$, and the
measures $\eta_{jk}$ and $\kappa_{jk}$. In Figure
\ref{fig:Spectral_Function_Symmetries} we demonstrate that 
this behavior of the spectral measures persists even in the finite
lattice setting.  Displayed in Figure
\ref{fig:Spectral_Function_Symmetries}(a) are transformations of the
spectral function for $\kappa_{22}$, associated with the effective
resistivity $\rho^*_{22}$. In particular, the spectral function of
$(1-\lambda)\kappa_{22}(1-\lambda)$ is displayed for volume fraction $p_1=0.1$, 0.3,
and 0.5, along with the spectral function of $\lambda\kappa_{22}(\lambda)$ for volume
fraction $1-p_1$. Similarly, in Figure
\ref{fig:Spectral_Function_Symmetries}(b) the spectral functions of   
$(1-\lambda)\mu_{33}(1-\lambda)$ and $\lambda\kappa_{33}(\lambda)$ are displayed for various values
of $p_1$ and $1-p_1$, respectively. The graphs of the transformed 
spectral functions are virtually identical except for delta
functions at $\lambda=0,1$, in excelent agreement with equation
...
% %
% \begin{figure}[]
%   \centerline{\includegraphics[scale=0.755]{2D_GammaCurl_and_3D_Gamma_Bounds.eps}} 
% \caption{asdf
%         }
...
\section{Conclusion}
%
We reviewed and extended the ACM for representing transport in
composites, which provides Stieltjes integral representations for the
effective parameters of two-phase random media, involving spectral 
measures of self-adjoint random operators. We have developed a
mathematical framework which provides a rigorous way to compute the
spectral measures and effective parameters of two-phase random media,
in the finite lattice setting. Moreover, we introduced a projection
method which provides a numerically efficient, stable way to   
...
%
In this appendix we review the spectral theorem, both for the bounded
linear self-adjoint operator case and the real-symmetric matrix
case. This discussion provides some of the omitted mathematical
details behind equations \eqref{eq:Stieltjes_F} and
\eqref{eq:Stieltjes_F_Discrete}. We also provide a review of the 
Stieltjes--Perron inversion theorem and discuss how it leads to
equation \eqref{eq:Measure_Relations}.  
...
theorem associated with the bounded linear self-adjoint operators
$M_i=\chi_i\Gamma\chi_i$ and $K_i=\chi_i\Upsilon\chi_i$, $i=1,2$, which arise naturally in the
ACM for the continuum and infinite lattice settings discussed in
...
theorem for the finite lattice setting, where the operators $M_i$ and
$K_i$ are represented by real-symmetric random matrices. In each case
we obtain Stieltjes integral representations for $\bsig^*$ and $\brho^*$,
...
%
In Section \eqref{sec:Continuum_Setting} we introduced Herglotz
...
representations. Due to the many symmetries between these functions
and measures, we focused on $m_{jk}(h)$ and $\mu_{jk}$, as the
discussions involving the other functions and measures follow by
direct analogy. For simplicity, we will continue this approach here. 
...
$\langle\cdot,\cdot\rangle_1=\langle\chi_1\,\cdot,\cdot\rangle$, $\Gamma\chi_1$ is a bounded linear self-adjoint
operator with spectrum contained in the interval $[0,1]$. By the
spectral theorem for such operators, there exists an increasing family
of self-adjoint projection operators $\{R(\lambda)\}$ - the resolution of the
identity - that satisfy $R(0)=0$ and $R(1)=I$ such that
% 
...
%
In this section we derive a discrete version of the integral
