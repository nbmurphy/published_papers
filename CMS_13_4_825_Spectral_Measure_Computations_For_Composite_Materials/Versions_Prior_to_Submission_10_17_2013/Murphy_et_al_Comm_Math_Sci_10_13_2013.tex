...
the symmetries and theoretical predictions of the models, and the computed values 
...
for the steady state, static case with real parameters. This work was extended
...
The analytic continuation method for addressing the 
effective parameter problem for \emph{two-component} media in the
quasi-static limit was developed by Bergman \cite{Bergman:PRL-1285},
Milton \cite{Milton:APL-300}, and Golden and Papanicolaou
\cite{Golden:CMP-473}.  The
...
involving random self-adjoint operators, leading to 
Stieltjes integral representations for the effective parameters.
This formulation demonstrated
...
equations \cite{Cherkaeva:IP-1203,MILTON:2002:TC,Cherkaev:2004:331,Cherkaev:PBCM:0921}, the
effective electrical, magnetic, and thermal transport properties are
\emph{all} completely determined by these measures.
...
\cite{Cherkaeva:IP-1203} when the data is given for a continuous
...
\cite{Cherkaeva:IP-1203}, effective thermal conductivity
\cite{Cherkaev:2004:331,Cherkaev:PBCM:0921}, and recover the associated structural parameters 
\cite{Cherkaeva:IP-1203,Cherkaeva:IP-065008,Zhang:JCP-5390,Bonifasi-Lista:PMB-3063,Cherkaev:JBiomech-345,Day:JPCM-99,Golden:JBM:337}. 
...
Highly accurate numerical computations of the
effective permittivity were used in \cite{Helsing:NJP:115005} to
estimate the spectral measures for continuum composites which have
sharp corners. The computations are based 
on a multigrid recursive compressed inverse preconditioning method
\cite{Helsing:2008:8820,Helsing:JCP:1171,Helsing:JCP:7533} developed
for calculation of the effective complex conductivity of random
checkerboards. In \cite{Day:JPCM-96} the effective complex
conductivity of the 2D random resistor network (RRN) percolation model
was computed using an efficient algorithm that implements $Y$-$\Delta$
transformations of the 2D network. In
...
associated with the composite microstructures of interest were computed
using the Stieltjes--Perron 
inversion theorem. However, the presence of delta components or
essential singularities in the measures, for example, makes it
difficult to evince details of the spectrum using this approach. To help
overcome this limitation, here we develop a mathematical framework
which provides a rigorous way to directly compute the spectral
measures and effective parameters for finite lattice composite
microstructures, or discretizations of continuum composites. In this
case the random operators underlying the integral representations of
the effective parameters are represented by random matrices, and the
spectral measures are determined explicitly by their eigenvalues and
eigenvectors. Our method provides a way to directly compute the
spectral measure and the effective parameters of such composite
media, and enables statistical analysis of the spectral data.
...
$\vec{\xi}^{\; T}P$ is the vector $\vec{\xi}^{\; T}$ with the entries permuted in
...
\cite{Halmos-1958}. Moreover, since 
...
\cite{Demmel:1997,Halmos-1958}. Consequently, equation
...
spectral endpoint $\lambda=0$ is displayed in (c) and (d).  The component
...
$\lambda=1$ is $\sim10^{-30}$ for both 2D and 3D. This behavior in the spectral
measure is consistent with equation \eqref{eq:Measure_Relations} which
holds for general stationary two-phase random media in the (infinite) 
...
DMS-1009704, ARC-0934721, DMS-0940249. We are also grateful for 
