...
We construct here a mathematical framework which unifies the critical
theory of transport in random two--phase composite media. We accomplish this
by adapting techniques developed by G. A. Baker for the Ising model
\cite{Baker-1990} to provide a detailed description of
percolation--driven critical transitions in transport. The most
natural formulation of this problem is in terms of the conduction
problem in the continuum $\mathbb{R}^d$, which includes the lattice
...
thermal conductivity, immediately generalize our results to all of
these systems.     
%------------------------------------------------------------------------
...
$T\to T_c^+$. Furthermore, he showed that the moments
$\psi_n=\int_0^\infty y^n\,d\psi(y)$, $n\geq0$,
%of $\psi$
diverge as $T\to T_c^+$ according to the
power law $\psi_n\sim(T-T_c)^{-\gamma_n}$. 
%Using a Stieltjes function characterization theorem \cite{Baker-1990},
He proved that the sequence $\gamma_n$ satisfies Baker's inequalities
...
%Lee--Yang--Ruelle--Baker
Baker's critical theory may be adapted to
...
the contrast ratio $h$ mimics the applied magnetic field $H$. More
specifically, the critical insulator/conductor behavior in transport
arises when $h=0$ $(\sigma_1=0, \ 0<\sigma_2<\infty)$, as $p\to p_c^+$
\cite{Golden:PRL-3935}, and the non-magnetic/ferromagnetic
critical behavior of the Ising model arises when $H=0$, as $T\to T_c^+$
\cite{Christensen-2005}. Using these mathematical
parallels, he showed that the critical exponents of transport
...
function components of $\mu$ and $\alpha$ located at $\lambda=0,1$. We do so by
...
$\bm{\sigma}(\vec{x},\omega)$ be the local conductivity tensor and 
...
\end{align}
%
where $m$, $\tilde{m}$, $w$, and $\tilde{w}$ are
...
\cite{Reed-1980} yield \cite{Golden:CMP-473,Bergman:PRC-377}
% % 
...
on the Hilbert space $L^2(\Omega,P)$ and, for example, in the Hilbert
space $L^2(\Omega,P)$ with weight $\chi_2$ in the inner product, $\Gamma\chi_2$ is a
bounded self--adjoint operator \cite{Golden:CMP-473}. They are
compositions of projection operators on the associated Hilbert spaces
$\mathscr{H}_\times$ and $\mathscr{C}_\bullet$, and are consequently bounded by 1
in the underlying operator norm \cite{Rudin:87,Folland:95}. Equation
...
$\mathbf{K}_j$, $j=1,2$. The measures $\mu$, $\eta$, $\alpha$, and $\kappa$ are
...
\end{remark}
%
Lemma \ref{lem:Measure_consistency_condition} is the key result
of this section. 
%
...
the signed measure $\Phi_{n-1}(dy)$ is defined. Furthermore in equation
...
\includegraphics[width=29pc]{2-3-d_Random_Resistor_Networks.eps}%
