...
\title{The Ising Model and a General Theory of Critical
...
precisely at the percolation threshold $p_c$ and at $1-p_c$. This is
analogous to the Lee-Yang-Ruelle characterization of
...
$\Omega$, or
%by an ergodic theorem \cite{Golden:CMP-473} 
...
$\tilde{a}(\vec{u},\vec{v})=\vec{u}^{\,T}[\bm{\sigma}^{-1}]\,\vec{v}$
...
$\mathcal{A}_{ij}$ and Stieltjes functions $\mathcal{S}$ of the
form
%
...
\begin{align}\label{eq:Herglotz_Funs_sed_LYRB}
  F(s)=\mathcal{A}_{0,1}(s;\mu), \quad
  E(s)=\mathcal{A}_{0,1}(s;\eta), \quad
  G(t)=\mathcal{A}_{0,1}(t;\alpha), \quad
  H(t)=\mathcal{A}_{0,1}(t;\kappa).
\end{align}
%
...
% $\tilde{\lambda}_0:=\inf(\Sigma_\eta)$, $\tilde{\lambda}_1:=\sup(\Sigma_\eta)$, 
$\hat{\lambda}_0:=\inf(\Sigma_\alpha)$ $\hat{\lambda}_1:=\sup(\Sigma_\alpha)$,
%\check{\lambda}_0:=\inf(\Sigma_\kappa) \ \check{\lambda}_1:=\sup(\Sigma_\kappa).
and so forth.
%In order to illuminate the symmetries between the integral
%representations \eqref{eq:Integral_Reps} of equation
%\eqref{eq:Herglotz_Funs_sed_LYRB},
...
\eqref{eq:Herglotz_Funs_sed_LYRB} and \eqref{eq:var_subs_Fs}, the
...
\end{align}
%
for $n\geq0$, which are analogs of equation \eqref{eq:Gtau_inneq} of the
...
%from the symmetries in equations \eqref{eq:Herglotz_Funs_sed_LYRB}
%and \eqref{eq:mh_Stieltjes_rep}.  
...
$[\sigma^{-1}]^*$ are due to the formation of delta function components
...
subtract the said quantity in equation
\eqref{eq:g_ghat_relation_integral}, yielding    
% %
...
Now consider transport through a random resistor
...
These critical behaviors in transport are made more
...
identity which follows from the relation \eqref{eq:m_w_relation},
...
$0<|h|\ll1$. It follows that the relation \eqref{eq:m_w_relation}
