...
\begin{align}\label{eq:h_Domain}
  \mathcal{U}_\varepsilon=\{h\in\mathbb{C}: |h|<1 \text{ and } |h-h_0|>\varepsilon \text{
    for all } h_0\in[-1,0]\},
\end{align}
%
where $0<\varepsilon\ll1$. %For $h\in\mathcal{U}_\varepsilon$ $m(h)$, $\tilde{m}(h)$,
%$w(z(h))$, and $\tilde{w}(z(h))$ are analytic functions of $h$
%\cite{Golden:CMP-473}. 
...
\cite{Torquato:RHM-02} 
%
\begin{align}\label{eq:Stieltjes_Bounds}
 0\leq F(1),E(1)\leq1 \ \text{ and } \
 0<|F(s)|,|E(s)|<1, \quad
 0<|G(t)|,|H(t)|<\infty, 
 \text{ for } h\in\mathcal{U}_\varepsilon,
\end{align}
%
...
$\vec{J}_0=J_0\,\vec{j}_k$, where $\vec{e}_k$ and $\vec{j}_k$ are
standard basis vectors, for some $k=1,\ldots,d$. Using equations
...
representations of $F(s)$, $E(s)$, $G(t)$, and $H(t)$
\cite{Golden:CMP-473,Bergman:PRC-377,Bergman:AP-78}    
% 
...
$\mathscr{C}_\bullet$, respectively, and are consequently positive definite
...
involves spectral representations of resolvents involving these self
adjoint operators. The measures $\mu$, $\eta$, $\alpha$, and $\kappa$ are spectral
measures of the family of projections of these operators in the
respective $\langle\vec{e}_k,\vec{e}_k\rangle$ or $\langle\vec{j}_k,\vec{j}_k\rangle$ state
...
analytic for all $h\in\mathcal{U}_\varepsilon$ \cite{Golden:CMP-473}.  As $\mu$, $\eta$,
...
\begin{align}\label{eq:Herglotz_Inneq}
  ...
  \zeta=g(h),\tilde{g}(h),\hat{g}(h),\check{g}(h), \quad h\in\mathcal{U}_\varepsilon,
\end{align}
%
...
\eqref{eq:Herglotz_Inneq} hold for $h\in\mathcal{U}_\varepsilon\cap\mathbb{R}$, and the
last inequality holds for $h\in\mathcal{U}_\varepsilon$ such that $h_i\neq0$. 
...
From equations 
\eqref{eq:Herglotz_Funs_sed_LYRB}, \eqref{eq:phi_moments}, and
\eqref{eq:Herglotz_energy_Reps}, we see that the first two moments,
$\phi_0$ and $\phi_1$, of $\phi$ are identified with energy components:     
%
...
$\phi_0$ is bounded. We prove this in Lemma
\ref{lem:Measure_consistency_condition} below, and that the higher
moments $\phi_n$, $n\geq1$, become singular as $\lambda_1=\sup\{\Sigma_\mu\}\to1$.
...
\begin{align}\label{eq:g_ghat_relation}
  g(h)+h\hat{g}(h)=1,
  \qquad
  \tilde{g}(h)+h\check{g}(h)=1, \quad h\in\mathcal{U}_\varepsilon.
\end{align}
%
For $h\in\mathcal{U}_\varepsilon$, the functions $g(h)$, $\hat{g}(h)$,
...
\end{align}
%
When $h\in\mathcal{U}_\varepsilon$ such that $h_i\neq0$, the complex representation of
equation \eqref{eq:Diff_g_ghat_relation} is, for example,  
%In the complex quasi--static case, where $h\in\mathcal{U}_\varepsilon$ such that
...
\begin{lemma}\label{lem:L1_Yij}  
  ...
  $i,j\in\mathbb{N}$ satisfying $0<i\leq j-1$, we have $Y_{i,j}(h,y)\in
  L^1(\ph(dy))$, and for $0<i\leq j$,
  $Y_{i,j}(h,y)\in L^1(\phi(dy))$. Consequently (\cite{Folland:99} Theorem  
...
For $h\in\mathcal{U}_\varepsilon$ we have $0<\varepsilon<|h|=|s-1|/|s|$, which implies that 
$|s-1|>\varepsilon|s|>0$. By equation \eqref{eq:Stieltjes_Bounds} we also have
$0\leq F(1)=\int_0^1d\mu(\lambda)/(1-\lambda)\leq1$. For $i,j\in\mathbb{N}$ such that $j\geq i$ and
$j\geq i+1$, equation \eqref{eq:Yij_lambda} now implies \cite{Rudin:87} that
%
\begin{align*}
...
%  
   \|Y_{i,j}\|_{\ph}&\leq|s|^j(1-\hat{\lambda}_0)^i\hat{\lambda}_1^{j-i-1}\,\alpha_0
                          \sup_{\lambda\in[\hat{\lambda}_0,\hat{\lambda}_1]}|s-1+\lambda|^{-j}<\infty,
\end{align*}
%
...
$\lambda_1=\hat{\lambda}_1=1$. This concludes the proof of Lemma \ref{lem:L1_Yij}
$\Box$. 
%----------------------------------------------------------------------------
%
...
In the static limit, $h\in\mathcal{U}_\varepsilon\cap\mathbb{R}$, as $h\to0$ the
...
$h\in\mathcal{U}_\varepsilon$ such that $h_i\neq0$, we also assume the existence of
...
\begin{lemma}\label{lem:zero_gamma0}
  ...
  \begin{align*}
    ...
    &\mathbf{2)} \quad 0<\lim_{h\to0}\langle\chi_1\vec{E}\cdot\vec{E}_0\rangle/E_0^2<1 \
         \text{ for all } \ p\in[0,1], \ h\in\mathcal{U}_\varepsilon.
  \end{align*}
  %
\end{lemma}
%
