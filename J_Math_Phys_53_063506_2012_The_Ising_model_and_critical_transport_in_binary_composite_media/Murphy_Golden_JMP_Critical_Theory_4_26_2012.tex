...
when $h_i=0$. Moreover, $t=1-s$ then implies that $|t_i|>\varepsilon/2>0$ when
...
this case, it is possible that \eqref{eq:Integral_rep_g_ghat} may not
hold for all $n\geq0$.  
%
\begin{definition}\label{def:Bounded_diff_g}
%
Define $\mathcal{B}_{n_\upsilon}$ to be the class of composites underlying
the measure $\upsilon=\phi,\ph\,$, such that the functions $\mathcal{S}_{i,j}$ in
\eqref{eq:Integral_Reps} satisfy
$\lim_{|h|\to0}|\mathcal{S}_{n,n+1}(h;\upsilon)|<\infty$ for all $0\leq n\leq n_\upsilon$.  
%
\end{definition}
%
By the above discussion,
%\cite{Bruno:PRSLA-353,MILTON:2002:TC}
there exists $\mathcal{B}_{n_\upsilon}\neq\emptyset$, $\upsilon=\phi,\ph\,$, $n_\upsilon\geq0$, and by
\eqref{eq:Diff_g_ghat_relation_Integral} $n_\phi\geq n_{\ph}\,$. 
The class $\mathcal{B}_{n_\upsilon}$, $\upsilon=\phi,\ph\,$, will be used
extensively in the proof of Theorem \ref{thm:Crit_Theory_m_w} below.
...
like the random resistor network (RRN) \cite{Stauffer-92,Clerc:AP-191,Bergman:SSP-147}. In the
...
$\chi(p,0)=\partial m(p,0)/\partial h\sim(p-p_c)^{-\gamma}$, as $p\to p_c^+$.  When
...
\ref{def:Bounded_diff_g} we cited examples of composites
for which there is a \emph{gap} in the support of $\mu$ about
$\lambda=1$, and subsequently defined a class $\mathcal{B}_{n_\phi}$ of
composites for which $\lim_{h\to0}|\partial^ng/\partial h^n|$ is bounded for all
$n\leq n_\phi$, whereby \eqref{eq:Cond-Insul_Crit_Beh_pc} $n_\phi\geq0$. For every
$p>p_c$ we assume that our percolation model is  
of class $\mathcal{B}_{n_\phi}$, so that there is a gap 
$\theta_\mu\sim(p-p_c)^\Delta$ in the support $\Sigma_\mu$ of $\mu(d\lambda,p)$ around $h=0$ or $s=1$
which collapses as $p\to p_c^+$, or that any spectrum in this region
does not affect power law behavior
\cite{Golden:PRL-3935} (see section \ref{sec:EMT} for an example of
such a percolation model). Consequently, for every $p>p_c$ we can think of 
the support $\Sigma_\phi$ of $\phi(dy,p)$ as being contained in the interval
$[0,S(p))$, with $S(p)\sim(p-p_c)^{-\Delta}$ as $p\to p_c^+$. We demonstrated in 
...
\begin{align}\label{eq:Crit_Exponents_wh}
  ...
  &w(p_c,z(h))\sim h^{-1/ \dha }, && w_r(p_c,z(h))\sim |h|^{-1/ \dha_r },
     ...
     \notag\\
  &\hat{\chi}(p)\sim(p_c-p)^{-\gh^\prime}, && \ph_n\sim(p_c-p)^{-\gh_n^\prime},
...
\begin{theorem} \label{thm:Crit_Theory_m_w}
  Consider a percolation model that is of class $\mathcal{B}_{n_\phi}$
  for every $p>p_c$, and of class $\mathcal{B}_{n_{\ph}}$ for every $p<p_c$, for some
  $0\leq n_{\ph}\leq n_{\phi}$. Let $t$, $t_r$, $t_i$, $\delta$, $\delta_r$, $\delta_i$,
  $\gamma$, $\gamma_n$, $\Delta$, $\gamma^\prime$, $\gamma_n^\prime$, and $\Delta^\prime$ be defined as in equation
  \eqref{eq:Crit_Exponents_mh}, and $s$, $s_r$, $s_i$, $\dha$,
  $\dha_r$, $\dha_i$, $\gh^\prime$, $\gh_n^\prime$, $\Dh^\prime$, $\gh$, $\gh_n$, and
  $\Dh$ be defined as in equation \eqref{eq:Crit_Exponents_wh}. Then
  the following scaling relations hold:
%  
  \begin{align*}   
...
then imply that $\gh_n^\prime>0$ for all $n\geq0$. The analogy of 
\eqref{eq:phi_energy_relations} involving $\ph_1$, equation
...
From equation \eqref{eq:Herglotz_Inneq} we have, for
$p\in[0,1]$ and $n\geq0$, 
%
...
every $p>p_c$ and $p<p_c$, respectively, so that
$\lim_{h\to0}|\partial^ng(p,h)/\partial h^n|<\infty$ and
$\lim_{h\to0}|\partial^n\hat{g}(p,h)/\partial h^n|<\infty$ for every $p>p_c$, $n\leq n_\phi$, and
$p<p_c$, $n\leq n_{\ph}$, respectively (see definition
\ref{def:Bounded_diff_g}). By equations
\eqref{eq:Cond-Insul_Crit_Beh_pc} and
\eqref{eq:Cond-SuperCond_Crit_Beh_pc}, $n_{\ph},n_\phi\geq0$ and 
$\mathcal{B}_{n_\phi},\mathcal{B}_{n_{\ph}}\neq\emptyset$, and by equation
\eqref{eq:Diff_g_ghat_relation_Integral}, $n_\phi\geq n_{\ph}$. We therefore have         
% 
...
\ref{lem:asymp_Scaling_funs_x_to_0_p>pc} we have, for all $1\leq n\leq n_\phi$,
%
\begin{align*}
...
$\gamma_i-\gamma_{i-1}=\Delta$. When this is true for all $n\geq0$, this is consistent
with the absence of multifractal behavior for the bulk conductivity
$\sigma^*(p,h)=\sigma_2m(p,h)$ \cite{Stauffer-92}. 

Now let $0<p_c-p\ll1$. By equations \eqref{eq:Crit_Exponents_wh},
\eqref{eq:Diff_g}, and \eqref{eq:Scaling_fun_Def}, and Lemma
\ref{lem:asymp_Scaling_funs_x_to_0_p<pc} we have, for all $1\leq n\leq n_{\ph}$, 
%
\begin{align*}
  (p_c-p)^{-\gh_n}\sim\ph_n
...
gap $\gh^\prime_i-\gh^\prime_{i-1}=\Dh$. When this is true for all $n\geq0$, this
...
\ref{lem:s_t}. Indeed as $1/\delta+1/\dha=1$ in general, where
$\delta=\Delta/(\Delta-\gamma)=\Delta/t$ 
...
For lattice systems with a finite number $n$ of lattice
...
RRN, the diagonal components $\alpha_{kk}$ are positive measures of equal
mass $1/d$, while the $\alpha_{ik}$, $i\neq k$, are signed measures of zero mass,
up to numerical accuracy. In Figure 1 the mass of the measures has
been scaled to the volume fraction $p$. 

We now provide an analytical proof for the existence of spectral gaps
in $\alpha=\alpha_{kk}$ about the spectral endpoints $\lambda=0,1$ for arbitrary, finite
...
$\mathbf{M}_1=\chi_1\Gamma\chi_1$ and $\mu$ by symmetry. In this lattice setting, $\Gamma$  
...
%and weight $m_0=1/n$, for $k$ such that $\vec{e}_k^{\;T}\vec{\text{v}}_j\neq0$.
...
condition $n_\phi,n_{\ph}\geq1$, we linked the two sets of critical
exponents in \eqref{eq:Crit_Exponents_mh} and
