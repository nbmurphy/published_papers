...
$\hat{\chi}(p):=\partial w(p,z(0))/\partial h\sim(p-p_c)^{-\gh}$ as $p\to p_c^-$. Furthermore,
...
of the limit $h\to0$. The above proof does not preclude that either
$s_r=0$ or $s_i=0$ (either $\hat{\mathcal{R}}_0(\xh)=0$ or
$\hat{\mathcal{I}}_0(\xh)=0)$, but not both. Although, this is not
physically consistent \cite{Efros:PSSB-303}.  
...
independent of the path of the limit $h\to0$, and the possibility that
either $t_r=0$ or $t_i=0$, but not both, is not physically consistent
...
\begin{lemma}\label{lem:s_t}
  ...
  %
  \begin{align*}
    \mathbf{1)} \quad s+t=\Delta\,. \qquad 
    \mathbf{2)} \quad \delta=1/(1-1/\dha\,^\prime)\,.
   \end{align*}
   %  
 \end{lemma}
%
