...
fields in $\vec{x}\in\mathbb{R}^d$ and $\omega\in\Omega$. Here $\Omega$ is the set of all
...
fields $\mathscr{H}_s\subset L^2(\Omega,P)$, and the associated Hilbert spaces of
stationary curl free random fields $\mathscr{H}_\times\subset\mathscr{H}_s$ and
stationary divergence free random   fields
$\mathscr{H}_{\bullet}\subset\mathscr{H}_s$ \cite{Golden:CMP-473}:     
%
...
\begin{align}\label{eq:L1(phi)_bound_infinite_set}
   \int_{S_h}^{S}|Y_{i,j}(h,y)|d\phi(y)
      ...
      %&\sim\frac{1}{|h|^j}\int_{S_h}^{\hat{S}}
      %           \frac{1+y}{y}d\alpha\left(\frac{1}{1+y}\right)
      %            \notag\\
      %&\sim\frac{1}{|h|^j}\int_{1-\hat{\lambda}_h}^{1-\hat{\lambda}_0}\frac{[-d\alpha(1-\lambda)]}{\lambda}        
...
\ref{sec:Measure_Equivalences} we show that equation
\eqref{eq:g_ghat_relation} and The Stieltjes-Perron Inversion Theorem
\cite{,Henrici:1974:v3} allows one to construct a measure $\varrho$ which
links the measures $\mu$ and $\alpha$. In section
\ref{sec:Crit_Behav_of_Transport}, we use $\varrho$ to prove that critical
transitions in transport are caused by the formation of delta function
components of the spectral measures, $\mu(d\lambda)$ and $\alpha(d\lambda)$, at the
spectral endpoints $\lambda=0,1$. Moreover, in percolation models, we 
show that the critical transition (the formation of these delta
components) occurs precisely at the percolation threshold.
%
%
%
