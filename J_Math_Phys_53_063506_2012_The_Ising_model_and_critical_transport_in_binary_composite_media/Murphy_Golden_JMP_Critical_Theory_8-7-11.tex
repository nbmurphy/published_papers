...
where $\vec{u},\vec{v}\in\mathbb{R}^d$, are bounded and
...
averages $\vec{E}_0$ and $\vec{J}_0$, respectively. For the random
...
otherwise \cite{Golden:CMP-473}. By equations
\eqref{eq:Weak_Curl_Free_Variational_Form}--\eqref{eq:Weak_Div_Free_Variational_Form}
we have $\langle\vec{J}\cdot\vec{E}_f\rangle=\langle\vec{E}\cdot\vec{J}_f\rangle=0$, which leads to
the following reduced energy representations 
%
\begin{align}\label{eq:Reduced_System_Energy_Representations}
  \langle\vec{J}\cdot\vec{E}\rangle=\langle\vec{J}\rangle\cdot\vec{E}_0=\langle\vec{E}\rangle\cdot\vec{J}_0\,.
\end{align}
%
...
knowlege, the operator $\Upsilon$ is being introduced here for the first
time. Let $\mathscr{C}_{\bullet}\subset\mathscr{H}_{\bullet}$ denote the \emph{closure}
of the space of stationary divergence free random fields of Coulomb
gauge,
i.e. $\mathscr{C}_{\bullet}:=\overline{\{\vec{\zeta}\in\mathscr{H}_{\bullet}|\,\vec{\nabla}\cdot\vec{\zeta}=0
  \text{ weakly}\}}$ \cite{Jackson-1999}. On the Hilbert space
$\mathscr{C}_{\bullet}$, one can easily show that the operator 
$\Upsilon=\vec{\nabla}\times(-\Delta)^{-1}\vec{\nabla}\times$ is a projector \cite{Jackson-1999},
which based on convolution with the free-space Green's function for
$-\Delta$ \cite{Golden:CMP-473}, i.e. $\Upsilon:\mathscr{H}_s\mapsto\mathscr{H}_\bullet$ and
for every $\vec{\zeta}\in\mathscr{C}_\bullet$, we have $\Upsilon\vec{\zeta}=\vec{\zeta}$. The
operators $\mathbf{M}_1:=\chi_1\Gamma\chi_1$, $\mathbf{M}_2:=\chi_2\Gamma\chi_2$,
$\mathbf{K}_1:=\chi_1\Upsilon\chi_1$, and $\mathbf{K}_2:=\chi_2\Upsilon\chi_2$ are self adjoint
in the Hilbert space $L^2(\Omega,P)$ \cite{Golden:CMP-473}. They are
compositions of projection operators, and are hence are bounded by 1
in the underlying operator norm \cite{Folland:95}. 
...
\end{align}
%
In order to illuminate the symmetries between the functions in
\eqref{eq:Herglotz_Funs_sed_LYRB}, we have used the variable $s$ in
lieu of $t(s)=1-s$ and will henceforth do so. In equation
\eqref{eq:Herglotz_Funs_sed_LYRB}, the measures $\mu$, $\eta$, $\alpha$, and $\kappa$
are bounded positive measures which depend only on the geometry of the
medium \cite{Golden:CMP-473,Bergman:AP-78}, and are supported on
$\Sigma_\mu,\Sigma_\eta,\Sigma_\alpha,\Sigma_\kappa\subseteq[0,1]$, respectively. Here, $\lambda_0:=\inf(\Sigma_\mu)\in[0,1)$,
...
$\check{\lambda}_1:=\sup(\Sigma_\kappa)\in(0,1]$. Equation
\eqref{eq:Herglotz_Funs_sed_LYRB} involves spectral representations of
resolvents involving the bounded self adjoint operators,
$\mathbf{M}_j$ and $\mathbf{K}_j$, $j=1,2$. The measure $\mu$, $\eta$, $\alpha$, 
and $\kappa$ are spectral measures of the family of projections of these
operators in the $\langle\vec{e}_k,\vec{e}_k\rangle$, or $\langle\vec{j}_k,\vec{j}_k\rangle$, state
...
measure, through its moments, which depend on the correlation
functions of the medium \cite{Golden:CMP-473}. For example, 
$\mu_0:=\int_0^1\lambda^0d\mu(\lambda)=\langle\chi_1\rangle=p_1$, the volume fraction of material
...
sample to bound its structural parameters such as $p_1$
...
and $\kappa$. The formulas in equation \eqref{eq:Herglotz_Funs_sed_LYRB}
can be transformed in terms of Stieltjes functions \cite{Baker-1990}
of $h$ via the change of variables $s=1/(1-h)$ and $\lambda(y)=y/(y+1)\iff
y(\lambda)=\lambda/(1-\lambda)$ so that, for example,  
%
...
of $G(t(s))$ and $\alpha$. Furthermore, Theorem \ref{thm:Herglotz_Decomp_Energy}
identifies the first two moments of $\tilde{\phi}$ and $\check{\phi}$ with
energy components. In order to make connections to $F(s)$ and $G(t(s))$ in the
...
\subsection{Measure Equivalencies in
...
we have that
$\langle\vec{J}\cdot\vec{E}\rangle=\langle\vec{J}\rangle\cdot\vec{E}_0=\sigma_2m(h)E_0^2=\sigma_1w(z(h))E_0^2$.  
%
\begin{align}\label{eq:m_w_relation}
  m(h)=hw(z(h)) \iff  1-F(s)=(1-1/s)(1-G(t(s)))
\end{align}
%
To evoke this theorem directly we write $d\tilde{\alpha}(\lambda):=[-d\alpha(1-\lambda)]$
%
...
By equations
\eqref{eq:Weak_Curl_Free_Variational_Form}--\eqref{eq:Weak_Div_Free_Variational_Form},
we have that $\langle\vec{J}\cdot\vec{E}_f\rangle=0$ and $\langle\vec{E}\cdot\vec{J}_f\rangle=0$,
respectively. Therefore we have the following energy constraint on the
system $\langle\vec{E}\cdot\vec{J}\rangle =\langle\vec{E}\cdot\vec{J}_0\rangle$. 
% 
